...
if ($Lang) {
    if ($Lang -eq "A" -or $Lang -eq "all") {
        if ("readme" -eq "launcher") {
            Invoke-ModuleLauncherWindows $Languages40 -noWait $NoWait
            exit 0
        } else {
    ...
    if ($selectedList.Count -gt 0) {
        if ("readme" -eq "launcher") {
            Invoke-ModuleLauncherWindows $selectedList -noWait $NoWait
            exit 0
        } else {
            ...
            for ($idx = 0; $idx -lt $selectedList.Count; $idx++) {
                ...
                $noWaitFlag = if ($multi) { -not $isLast -or $NoWait } else { $NoWait }
                switch ("readme") {
                    "readme"          { Invoke-ModuleReadme $item -noWait $noWaitFlag }
                    "readme"  { Invoke-ModuleReadme $item -noWait $noWaitFlag }
                    "changelog_patch" { Invoke-ModuleChangelogPatch $item -noWait $noWaitFlag }
                    "script"          { Invoke-ModuleScriptPsbbn $item -noWait $noWaitFlag }
                    "system"          { Invoke-ModuleSystemPsbbn $item -noWait $noWaitFlag }
                }
            }
            exit 0
        }
...
function Show-StandaloneMenu {
    while ($true) {
        ...
        Write-Host ("  {0} {1}" -f $ui.SourceFile, "Readme\input\README.md") -ForegroundColor DarkGray
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        ...
        if ($choice -eq "A" -or $choice -eq "a") {
            foreach ($langObj in $Languages40) {
                Invoke-ModuleReadme $langObj -noWait $true
            }
            Write-Host ""
            Write-Host "  ========================================================================================" -ForegroundColor Green
        ...
        if ($selectedList.Count -gt 0) {
            ...
            for ($idx = 0; $idx -lt $selectedList.Count; $idx++) {
                ...
                Invoke-ModuleReadme $item -noWait $noWaitFlag
            }
            continue
        } else {
            Write-Host ""
