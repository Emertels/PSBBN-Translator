<#
========================================================================================
PSBBN LAUNCHER FOR WINDOWS TRANSLATOR - V1
...
function Is-ValidTranslation([string]$text) {
    if ([string]::IsNullOrWhiteSpace($text)) { return $false }
    if ($text.Length -eq 1 -and $text -match '^[a-zA-Z0-9]$') { return $false }
    if ($text -match '^(<!DOCTYPE|<html|<head|<body|<script|<div|<span|<title|HTTP/|403\s+Forbidden|404\s+Not|500\s+Internal)') { return $false }
...
function Invoke-SingleTranslation($text, $targetGoogleCode) {
    ...
    try {
        ...
        $html2 = $r2.ReadToEnd()
        $r2.Close(); $res2.Close()
        $m2 = [regex]::Match($html2, '(?s)<div[^>]*class="result-container"[^>]*>(.*?)</div>')
        if ($m2.Success) {
            $c2 = [System.Net.WebUtility]::HtmlDecode($m2.Groups[1].Value.Trim())
            if (Is-ValidTranslation $c2) { return $c2 }
        }
    } catch { }
    ...
    try {
        ...
        $req3.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        $req3.Timeout = 8000
        ...
        if ($parsed3 -and $parsed3[0]) {
            $sb3 = New-Object System.Text.StringBuilder
            foreach ($seg3 in $parsed3[0]) {
                if ($seg3[0]) { [void]$sb3.Append($seg3[0]) }
            }
            $resStr3 = $sb3.ToString()
            if (Is-ValidTranslation $resStr3) { return $resStr3 }
        }
    } catch { }
...
function Show-StandaloneMenu {
    while ($true) {
        $ui = Get-ScriptUI
        $title = if ($ui.LauncherTitle) { $ui.LauncherTitle } else { "PSBBN Launcher for Windows Translator [By Emerson Teles]" }
        $desc = if ($ui.DescLauncher) { $ui.DescLauncher } else { "Adiciona suporte e deteção de até 40 idiomas no PSBBN Launcher for Windows." }
        ...
        Write-Host ("  {0} {1}" -f $ui.SourceFile, "Launcher_Windows\input\PSBBN-Launcher-For-Windows.ps1") -ForegroundColor DarkGray
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        ...
        Write-Host ("  [0] {0}" -f $ui.ExitOption) -ForegroundColor Yellow
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        ...
        Write-Host ""
        $choice = Read-Host ("  {0}" -f $ui.LauncherPrompt)
        $choice = $choice.Trim()
