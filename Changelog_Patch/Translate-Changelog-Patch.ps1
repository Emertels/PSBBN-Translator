<#
========================================================================================
PSBBN CHANGELOG PATCH TRANSLATOR - V1
...
function Apply-TermReplacements($text, $targetLangObj, [bool]$isChangelog = $false) {
    ...
    $res = $res -replace '(?i)\b(?:redefinição\s+no\s+jogo|in-game\s+reset)\s*\(IGR\)', 'In-Game Reset (IGR)'

    # Homebrews Protection
    $res = $res -replace '(?i)\bOpen\s+PS2\s+Loader\s*\(OPL\)', 'Open PS2 Loader (OPL)'
    $res = $res -replace '(?i)\bAbra\s+o\s+carregador\s+do\s+PS2\b', 'Open PS2 Loader'
    $res = $res -replace '(?i)\bAbra\s+o\s+carregador\s+PS2\b', 'Open PS2 Loader'
    $res = $res -replace '(?i)\bAbra\s+carregador\s+ps2\b', 'Open PS2 Loader'
...
function Show-StandaloneMenu {
    while ($true) {
        ...
        Write-Host ("  {0} {1}" -f $ui.SourceFile, "Changelog_Patch\input\changelog_patch_eng.txt") -ForegroundColor DarkGray
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        ...
        if ($choice -eq "A" -or $choice -eq "a") {
            foreach ($langObj in $Languages40) {
                Invoke-ModuleChangelogPatch $langObj -noWait $true
            }
            Write-Host ""
            Write-Host "  ========================================================================================" -ForegroundColor Green
        ...
        if ($selectedList.Count -gt 0) {
            ...
            for ($idx = 0; $idx -lt $selectedList.Count; $idx++) {
                ...
                Invoke-ModuleChangelogPatch $item -noWait $noWaitFlag
            }
            continue
        } else {
            Write-Host ""
