...
function Apply-TermReplacements($text, $targetLangObj, [bool]$isChangelog = $false) {
    ...
    # Main Menu translation across languages (prevents Google detecting 'Main' as Malay 'play')
    $mainMenuTrans = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "Menu Principal" }
        "es" { [regex]::Unescape("Men\u00FA Principal") }
        "fr" { "Menu Principal" }
        "it" { "Menu Principale" }
        "de" { [regex]::Unescape("Hauptmen\u00FC") }
        { $_ -in @("tl", "fil") } { "Pangunahing Menu" }
        "ru" { [regex]::Unescape('\u0413\u043B\u0430\u0432\u043D\u043E\u0435 \u043C\u0435\u043D\u044E') }
        "ja" { [regex]::Unescape('\u30E1\u30A4\u30F3\u30E1\u30F8\u30E5\u30FC') }
        default { "Main Menu" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?MAIN_?MENU\s*(_\s*)?XYZ', $mainMenuTrans
    $res = $res -replace '(?i)\b(?:Play\s+Menu|Menu\s+Play)\b', $mainMenuTrans
    if ($targetLangObj.code -in @("tl", "fil")) {
        $res = $res -replace '(?i)\b(?:Main\s+Menu|Play\s+Menu)\b', 'Pangunahing Menu'
    } elseif ($targetLangObj.code -in @("pt", "pt-pt")) {
        $res = $res -replace '(?i)\b(?:Main\s+Menu|Play\s+Menu)\b', 'Menu Principal'
    } elseif ($targetLangObj.code -eq "es") {
        $res = $res -replace '(?i)\b(?:Main\s+Menu|Play\s+Menu)\b', [regex]::Unescape("Men\u00FA Principal")
    }

    # Video demonstration of PSBBN across languages
    $videoDemoTrans = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { [regex]::Unescape("Demonstra\u00E7\u00E3o em V\u00EDdeo do PSBBN") }
        "es" { [regex]::Unescape("Demostraci\u00F3n en Video de PSBBN") }
        "fr" { [regex]::Unescape("D\u00E9monstration Vid\u00E9o de PSBBN") }
        "it" { "Dimostrazione Video di PSBBN" }
        "de" { "Videodemonstration von PSBBN" }
        { $_ -in @("tl", "fil") } { "Pagpapakita ng Video ng PSBBN" }
        default { "Video demonstration of PSBBN" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?VIDEO_?DEMO_?PSBBN\s*(_\s*)?XYZ', $videoDemoTrans
    $res = $res -replace '(?i)\bVideo\s+Demonstration\s+(?:Ng|of|de)\s+PSBBN\b', $videoDemoTrans
    $res = $res -replace '(?i)\bDemonstration\s+of\s+PSBBN\b', $videoDemoTrans

    # Truncated or mangled token protection
    $res = $res -replace 'XYZ_HDD_OSD_ICON\b', 'HDD-OSD Icon Database'
    $res = $res -replace 'XYZ_HDD_OSD_ICON_D_XYMDX\b', 'https://github.com/CosmicScale/HDD-OSD-Icon-Database'
    $res = $res -replace 'XYZ\s*(_\s*)?HDD_?OSD_?IC(O)?N[A-Z0-9_]*', 'HDD-OSD Icon Database'

    # 1. Protect & Restore PSBBN Definitive Project strictly in English across all 40 languages
    $res = $res -replace 'XYZ\s*(_\s*)?PSBBN_?DEF_?PROJ(ECT)?\s*(_\s*)?XYZ', 'PSBBN Definitive Project'
    $res = $res -replace '(?i)\b(Projeto Definitivo PSBBN|Projecto Definitivo PSBBN|Projet D[eÃ©]finitif PSBBN|Definitives PSBBN-Projekt|Definitiv PSBBN-Projekt|Proyecto Definitivo PSBBN|Progetto Definitivo PSBBN|Definitive PSBBN Project)\b', 'PSBBN Definitive Project'
    $res = $res -replace '(?i)\b(\u041E\u043A\u043E\u043D\u0447\u0430\u0442\u0435\u043B\u044C\u043D\u044B\u0439\s+\u043F\u0440\u043E\u0435\u043A\u0442\s+PSBBN)\b', 'PSBBN Definitive Project'

    # PSBBN Definitive Patch restoration
    $res = $res -replace 'XYZ\s*(_\s*)?PSBBN_?DEF_?PATCH\s*(_\s*)?XYZ', 'PSBBN Definitive Patch'
    $res = $res -replace '(?i)\b(?:Patch Definitivo PSBBN|Patch Definitivo do PSBBN)\b', 'PSBBN Definitive Patch'

    # OSDMenu Configurator restoration
    $res = $res -replace 'XYZ\s*(_\s*)?OSDMENU_?CONFIG\s*(_\s*)?XYZ', 'OSDMenu Configurator'
    ...
    # OSDMenu MBR restoration
    $res = $res -replace 'XYZ\s*(_\s*)?OSDMENU_?MBR\s*(_\s*)?XYZ', 'OSDMenu MBR'
    $res = $res -replace '(?i)\b(?:MenuOSD\s*MBR|MenuOSDMBR)\b', 'OSDMenu MBR'
    $res = $res -replace '(?i)\bMenuOSD\b', 'OSDMenu'

    # Navigator Menu & Navigator term restoration
    $res = $res -replace 'XYZ\s*(_\s*)?NAVIGATOR_?TOKEN\s*(_\s*)?XYZ', 'Navigator'
    $navMenuTarget = if ($targetLangObj.code -in @("pt", "pt-pt", "es", "fr", "it")) { "Menu Navigator" } elseif ($targetLangObj.code -eq "de") { "Navigator-MenÃ¼" } else { "Navigator Menu" }
    $res = $res -replace 'XYZ\s*(_\s*)?NAVIGATOR_?MENU\s*(_\s*)?XYZ', $navMenuTarget

    # Portuguese Ativar/Desativar standardizations (never use "Habilitar")
    if ($targetLangObj.code -in @("pt", "pt-pt")) {
        ...
        $res = $res -replace '(?i)\bdesabilitad([oa]s?)\b', 'desativad$1'
        $res = $res -replace '(?i)\bHabilitando\b', 'Ativando'
        $res = $res -replace '(?i)\bhabilitando\b', 'ativando'
        $res = $res -replace '(?i)\bHabilita\b', 'Ativa'
        $res = $res -replace '(?i)\bhabilita\b', 'ativa'
        $res = $res -replace '(?i)\bHabilita[Ã§c][Ã£a]o\b', 'AtivaÃ§Ã£o'
        $res = $res -replace '(?i)\bhabilita[Ã§c][Ã£a]o\b', 'ativaÃ§Ã£o'
        $res = $res -replace '(?i)\bDesabilitando\b', 'Desativando'
        $res = $res -replace '(?i)\bdesabilitando\b', 'desativando'
        $res = $res -replace '(?i)\bDesabilita\b', 'Desativa'
        $res = $res -replace '(?i)\bdesabilita\b', 'desativa'
        $res = $res -replace '(?i)\bDesabilita[Ã§c][Ã£a]o\b', 'DesativaÃ§Ã£o'
        $res = $res -replace '(?i)\bdesabilita[Ã§c][Ã£a]o\b', 'desativaÃ§Ã£o'
    ...
    $res = $res -replace 'XYZ\s*(_\s*)?PSBBN_?LAUNCH_?WIN\s*(_\s*)?XYZ', 'PSBBN Launcher for Windows'

    # README Term Protection
    $res = $res -replace 'XYZ\s*(_\s*)?README_?TOKEN\s*(_\s*)?XYZ', 'README'

    # HOSDMenu protection
    $res = $res -replace 'XYZ\s*(_\s*)?HOSDMENU\s*(_\s*)?XYZ', 'HOSDMenu'

    # Art Packs protection
    $res = $res -replace 'XYZ\s*(_\s*)?PSBBN_?ART_?DB\s*(_\s*)?XYZ', 'PSBBN Art Database'
    $res = $res -replace 'XYZ\s*(_\s*)?HDD_?OSD_?ICON_?DB\s*(_\s*)?XYZ', 'HDD-OSD Icon Database'
    $res = $res -replace 'XYZ\s*(_\s*)?OPL_?BAPPS_?PACK\s*(_\s*)?XYZ', 'OPL B-APPS Cover Pack'
    $res = $res -replace 'XYZ\s*(_\s*)?OPL_?DISCS_?PACK\s*(_\s*)?XYZ', 'OPL Discs & Boxes Pack'
    $res = $res -replace 'XYZ\s*(_\s*)?OPL_?MGR_?ART_?DB\s*(_\s*)?XYZ', 'OPL Manager Art Database'
    $res = $res -replace '(?i)\bPacotes?\s+de\s+artes?\b', 'Art Packs'

    # Official Systems Protection
    $res = $res -replace 'XYZ\s*(_\s*)?SAVE_?APP_?SYSTEM\s*(_\s*)?XYZ', 'Save Application System'
    $res = $res -replace '(?i)\bSistema\s+de\s+Aplicativos?\s+Salvos?\s*\((?:SAS|SAS-compliant)\)', 'Save Application System (SAS)'

    $res = $res -replace 'XYZ\s*(_\s*)?IN_?GAME_?RESET\s*(_\s*)?XYZ', 'In-Game Reset'
    $res = $res -replace '(?i)\b(?:redefiniÃ§Ã£o\s+no\s+jogo|in-game\s+reset)\s*\(IGR\)', 'In-Game Reset (IGR)'

    # Homebrews & Tools Protection
    $res = $res -replace 'XYZ\s*(_\s*)?OPEN_?PS2_?LOADER\s*(_\s*)?XYZ', 'Open PS2 Loader'
    $res = $res -replace '(?i)\bOpen\s+PS2\s+Loader\s*\(OPL\)', 'Open PS2 Loader (OPL)'
    $res = $res -replace '(?i)\b(?:Buksan\s+ang\s+PS2\s+Loader|Abra\s+o\s+carregador\s+do\s+PS2|Abra\s+o\s+carregador\s+PS2|Abra\s+carregador\s+ps2|Ouvrir\s+le\s+chargeur\s+PS2|[Ã–Ã¶]ffnen\s+Sie\s+den\s+PS2-Loader|Apri\s+il\s+caricatore\s+PS2|Abrir\s+el\s+cargador\s+PS2)\b', 'Open PS2 Loader'

    # APA-Jail protection
    $res = $res -replace 'XYZ\s*(_\s*)?APA_?JAIL\s*(_\s*)?XYZ', 'APA-Jail'
    $res = $res -replace '(?i)\b(?:Kulungan\s+ng\s+APA|Pris[Ã£a]o\s+APA|Prison\s+APA|APA-Gef[Ã¤a]ngnis|Prigione\s+APA|C[Ã¡a]rcel\s+APA)\b', 'APA-Jail'

    # Power Controls
    $res = $res -replace 'XYZ\s*(_\s*)?MAIN_?POWER\s*(_\s*)?XYZ', 'MAIN POWER'
    $res = $res -replace '(?i)\bMAIN\s+POWER\b', 'MAIN POWER'
    $res = $res -replace 'XYZ\s*(_\s*)?ON_?STANDBY_?RESET\s*(_\s*)?XYZ', 'ON/STANDBY/RESET'
    $res = $res -replace '(?i)\bON/STANDBY/RESET\b', 'ON/STANDBY/RESET'

    # Webhook & Ko-fi
    $res = $res -replace 'XYZ\s*(_\s*)?KO_?FI\s*(_\s*)?XYZ', 'Ko-fi'
    $res = $res -replace 'XYZ\s*(_\s*)?DISCORD_?WEBHOOK\s*(_\s*)?XYZ', 'Discord Webhook'
    $res = $res -replace 'XYZ\s*(_\s*)?WEBHOOK\s*(_\s*)?XYZ', 'Webhook'

    # Portuguese (Portugal) vocabulary adjustments
    if ($targetLangObj.code -eq "pt-pt") {
        $res = $res -replace '(?i)\baplicativos\b', 'aplicações'
        $res = $res -replace '(?i)\baplicativo\b', 'aplicação'
        $res = $res -replace '(?i)\barquivos\b', 'ficheiros'
        $res = $res -replace '(?i)\barquivo\b', 'ficheiro'
        $res = $res -replace '(?i)\btela\b', 'ecrã'
        $res = $res -replace '(?i)\btelas\b', 'ecrãs'
    }


    # Virtual Memory Cards (VMCs) Multilingual Terminology
    $vmcWithAcronym = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "CartÃµes de MemÃ³ria Virtuais (VMCs)" }
        "es" { "Tarjetas de Memoria Virtuales (VMCs)" }
        "fr" { "Cartes MÃ©moire Virtuelles (VMC)" }
        "de" { "Virtuelle Speicherkarten (VMCs)" }
        "it" { "Schede di Memoria Virtuali (VMC)" }
        "tl" { "Mga Virtual Memory Card (VMC)" }
        "ru" { [regex]::Unescape('\u0412\u0438\u0440\u0442\u0443\u0430\u043B\u044C\u043D\u044B\u0435 \u043A\u0430\u0440\u0442\u044B \u043F\u0430\u043C\u044F\u0442\u0438 (VMC)') }
        "ja" { [regex]::Unescape('\u4EEE\u60F3\u30E1\u30E2\u30EA\u30FC\u30AB\u30FC\u30C9\uFF08VMC\uFF09') }
        "ar" { [regex]::Unescape('\u0628\u0637\u0627\u0642\u0627\u062A \u0627\u0644\u0630\u0627\u0643\u0631\u0629 \u0627\u0644\u0627\u0641\u062A\u0631\u0627\u0636\u064A\u0629 (VMCs)') }
        default { "Virtual Memory Cards (VMCs)" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?VMCS_?WITH_?ACRONYM\s*(_\s*)?XYZ', $vmcWithAcronym

    $vmcPlural = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "CartÃµes de MemÃ³ria Virtuais" }
        "es" { "Tarjetas de Memoria Virtuales" }
        "fr" { "Cartes MÃ©moire Virtuelles" }
        "de" { "Virtuelle Speicherkarten" }
        "it" { "Schede di Memoria Virtuali" }
        "tl" { "Mga Virtual Memory Card" }
        default { "Virtual Memory Cards" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?VIRTUAL_?MEM_?CARDS\s*(_\s*)?XYZ', $vmcPlural

    $vmcSingular = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "CartÃ£o de MemÃ³ria Virtual" }
        "es" { "Tarjeta de Memoria Virtual" }
        "fr" { "Carte MÃ©moire Virtuelle" }
        "de" { "Virtuelle Speicherkarte" }
        "it" { "Scheda di Memoria Virtuale" }
        "tl" { "Virtual Memory Card" }
        default { "Virtual Memory Card" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?VIRTUAL_?MEM_?CARD\s*(_\s*)?XYZ', $vmcSingular

    $vmcGroups = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "Grupos de VMC" }
        "es" { "Grupos de VMC" }
        "fr" { "Groupes de VMC" }
        "de" { "VMC-Gruppen" }
        "it" { "Gruppi di VMC" }
    ...
    $res = $res -replace 'XYZ\s*(_\s*)?VMC_?GROUPS\s*(_\s*)?XYZ', $vmcGroups

    $gameAppInstaller = switch ($targetLangObj.code) {
        { $_ -in @("pt", "pt-pt") } { "Instalador de Jogos e Aplicativos" }
        "es" { "Instalador de Juegos y Aplicaciones" }
        "fr" { "Installateur de Jeux et d'Applications" }
        "de" { "Spiele- und App-Installer" }
        "it" { "Programma di Installazione di Giochi e App" }
        "tl" { "Installer ng Laro at App" }
        default { "Game and App Installer" }
    }
    $res = $res -replace 'XYZ\s*(_\s*)?GAME_?APP_?INSTALLER\s*(_\s*)?XYZ', $gameAppInstaller

    # SMB Network Protocol
    $res = $res -replace 'XYZ\s*(_\s*)?SMB_?TOKEN\s*(_\s*)?XYZ', 'SMB'
    $res = $res -replace '(?i)\b(?:Pequenas\s+e\s+M[Ã©e]dias\s+Empresas|PME)\b', 'SMB'
    ...
    $res = $res -replace '(?i)\bLanÃ§amento\s+de\s+Jogos\s+PS1\s+via\s+SMB\b', 'ExecuÃ§Ã£o de Jogos PS1 via SMB'
    $res = $res -replace '(?i)\bLanÃ§amento\s+de\s+Jogos\s+PS1\s+de\s+SMB\b', 'ExecuÃ§Ã£o de Jogos PS1 via SMB'

    # ATA BDM Assault Driver Restoration
    $res = $res -replace 'XYZ\s*(_\s*)?ATA_?BDM_?ASSAULT\s*(_\s*)?XYZ', 'ATA BDM Assault'
    ...
    $res = $res -replace '(?i)\bInstalando\s+o\s+Ataque\s+ATA\s+BDM\b', 'InstalaÃ§Ã£o dos Drivers ATA BDM Assault'
    $res = $res -replace '(?i)\bInstalando\s+ATA\s+BDM\s+Assault\b', 'InstalaÃ§Ã£o dos Drivers ATA BDM Assault'

    # Changelog Channel Tokens Restoration
    ...
    # 2. PlayStation Button Actions & Nuance
    ...
    # 4. Standard UI & Menu terms
    if ($targetLangObj.installGames) {
        $res = $res -replace '(?i)[\u201C\u201D"''Â«Â»]Install\s+Games\s+and\s+Apps[\u201C\u201D"''Â«Â»]', ("`"{0}`"" -f $targetLangObj.installGames)
        $res = $res -replace '(?i)\bInstall\s+Games\s+and\s+Apps\b', $targetLangObj.installGames
    }
    if ($targetLangObj.optExtras) {
        $res = $res -replace '(?i)[\u201C\u201D"''Â«Â»]Optional\s+Extras[\u201C\u201D"''Â«Â»]', ("`"{0}`"" -f $targetLangObj.optExtras)
        $res = $res -replace '(?i)\bOptional\s+Extras(?:\s+menu)?\b', $targetLangObj.optExtras
    }
    if ($targetLangObj.gameSelector) {
        $res = $res -replace '(?i)[\u201C\u201D"''Â«Â»]Game\s+Selector[\u201C\u201D"''Â«Â»]', ("`"{0}`"" -f $targetLangObj.gameSelector)
        $res = $res -replace '(?i)\bGame\s+Selector\b', $targetLangObj.gameSelector
    }

    # 5. Comprehensive fail-safe sweep for all protected terms so NO XYZ token ever leaks
    $res = $res -replace '(?i)XYZ\s*(_\s*)?PSBBN_?DEF_?PROJ(ECT)?\s*(_\s*)?XYZ', 'PSBBN Definitive Project'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?PSBBN_?DEF_?PATCH\s*(_\s*)?XYZ', 'PSBBN Definitive Patch'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OSDMENU_?CONFIG\s*(_\s*)?XYZ', 'OSDMenu Configurator'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OSDMENU_?MBR\s*(_\s*)?XYZ', 'OSDMenu MBR'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?PSBBN_?LAUNCH_?WIN\s*(_\s*)?XYZ', 'PSBBN Launcher for Windows'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?HOSDMENU\s*(_\s*)?XYZ', 'HOSDMenu'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?HDD_?OSD_?IC(O)?N[A-Z0-9_]*', 'HDD-OSD Icon Database'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OPL_?MGR_?ART_?DB\s*(_\s*)?XYZ', 'OPL Manager Art Database'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?PSBBN_?ART_?DB\s*(_\s*)?XYZ', 'PSBBN Art Database'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?SMB_?TOKEN\s*(_\s*)?XYZ', 'SMB'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?README_?TOKEN\s*(_\s*)?XYZ', 'README.md'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?DISCORD_?WEBHOOK\s*(_\s*)?XYZ', 'Discord Webhook'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?WEBHOOK\s*(_\s*)?XYZ', 'Webhook'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?KO_?FI\s*(_\s*)?XYZ', 'Ko-fi'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?VMCS_?WITH_?ACRONYM\s*(_\s*)?XYZ', 'Virtual Memory Cards (VMCs)'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?VIRTUAL_?MEM_?CARDS?\s*(_\s*)?XYZ', 'Virtual Memory Cards'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?VMC_?GROUPS\s*(_\s*)?XYZ', 'VMC Groups'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?GAME_?APP_?INSTALLER\s*(_\s*)?XYZ', 'Game and App Installer'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OPEN_?PS2_?LOADER\s*(_\s*)?XYZ', 'Open PS2 Loader'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?SAVE_?APP_?SYSTEM\s*(_\s*)?XYZ', 'Save Application System'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?IN_?GAME_?RESET\s*(_\s*)?XYZ', 'In-Game Reset'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?APA_?JAIL\s*(_\s*)?XYZ', 'APA-Jail'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?MAIN_?POWER\s*(_\s*)?XYZ', 'MAIN POWER'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?ON_?STANDBY_?RESET\s*(_\s*)?XYZ', 'ON/STANDBY/RESET'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?NAVIGATOR_?MENU\s*(_\s*)?XYZ', 'Navigator Menu'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?ATA_?BDM_?ASSAULT\s*(_\s*)?XYZ', 'ATA BDM Assault'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OPL_?BAPPS_?PACK\s*(_\s*)?XYZ', 'OPL B-APPS Cover Pack'
        $res = $res -replace '(?i)XYZ\s*(_\s*)?OPL_?DISCS_?PACK\s*(_\s*)?XYZ', 'OPL Discs & Boxes Pack'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?PSBBN(_\s*TOKEN)?\s*(_\s*)?XYZ', 'PSBBN'
    $res = $res -replace '(?i)\bXYZ_PSBBN\b', 'PSBBN'
    $res = $res -replace '(?i)\bPSBBN\s+TOKEN\b', 'PSBBN'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?APA[A-Z0-9_]*', 'APA-Jail'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?OSDMENU_?MBR[A-Z0-9_]*', 'OSDMenu MBR'
    $res = $res -replace '(?i)XYZ\s*(_\s*)?ATA_?BDM[A-Z0-9_]*', 'ATA BDM Assault'
...
# ------------------------------------------------------------------------------
# Markdown Syntax Tokenizer & Term Shielding (CosmicScale Feedback & Link Safety)
# ------------------------------------------------------------------------------
function Tokenize-MarkdownTokens([string]$text, [ref]$tokensMap) {
    if ([string]::IsNullOrWhiteSpace($text)) { return $text }
    $map = @{
    ...
    $c = [regex]::Replace($c, '\]\(([^)\r\n]+)\)', {
        param($m)
        $idx = $map.urls.Count
        $map.urls.Add($m.Groups[1].Value)
        return "](https://u${idx}.link)"
    })

    # 3. Raw URLs: https?://... -> https://rN.link
    $c = [regex]::Replace($c, 'https?://(?!u\d+\.link)[^\s<>"]+', {
        param($m)
        $idx = $map.raws.Count
        $map.raws.Add($m.Value)
        return "https://r${idx}.link"
    })

    # 4. HTML tags: <tag ...> or </tag>
    $c = [regex]::Replace($c, '</?(?:span|div|a|img|p|br|table|tr|td|th|details|summary|h[1-6])(?:\s+[^>]*)?>', {
        param($m)
        $idx = $map.tags.Count
        $map.tags.Add($m.Value)
        return "XYZHTMLTAG_${idx}_XYZ"
    })
...
function Protect-TermsBeforeTranslation([string]$text) {
    ...
    $c = $c -replace 'PSBBN Definitive Patch', 'XYZ_PSBBN_DEF_PATCH_XYZ'
    $c = $c -replace 'OSDMenu Configurator', 'XYZ_OSDMENU_CONFIG_XYZ'
    $c = $c -replace 'OSDMenu MBR', 'XYZ_OSDMENU_MBR_XYZ'
    $c = $c -replace 'PSBBN Launcher for Windows', 'XYZ_PSBBN_LAUNCH_WIN_XYZ'
    $c = $c -replace '\bHOSDMenu\b', 'XYZ_HOSDMENU_XYZ'
    $c = $c -replace '\bPSBBN\b', 'XYZ_PSBBN_TOKEN_XYZ'
    $c = $c -replace 'Navigator Menu', 'XYZ_NAVIGATOR_MENU_XYZ'
    $c = $c -replace 'ATA BDM Assault', 'XYZ_ATA_BDM_ASSAULT_XYZ'

    # 2. Homebrews & Systems
    $c = $c -replace 'Open PS2 Loader', 'XYZ_OPEN_PS2_LOADER_XYZ'
    $c = $c -replace 'Save Application System', 'XYZ_SAVE_APP_SYSTEM_XYZ'
    $c = $c -replace 'In-Game Reset', 'XYZ_IN_GAME_RESET_XYZ'
    $c = $c -replace 'APA-Jail', 'XYZ_APA_JAIL_XYZ'

    # 3. Hardware & Power Controls
    $c = $c -replace 'MAIN POWER', 'XYZ_MAIN_POWER_XYZ'
    $c = $c -replace 'ON/STANDBY/RESET', 'XYZ_ON_STANDBY_RESET_XYZ'

    # 4. Features & Terminology
    $c = $c -replace 'Virtual Memory Cards \(VMCs\)', 'XYZ_VMCS_WITH_ACRONYM_XYZ'
    $c = $c -replace 'Virtual Memory Cards', 'XYZ_VIRTUAL_MEM_CARDS_XYZ'
    $c = $c -replace 'Virtual Memory Card', 'XYZ_VIRTUAL_MEM_CARD_XYZ'
    $c = $c -replace 'VMC Groups', 'XYZ_VMC_GROUPS_XYZ'
    $c = $c -replace 'Game and App Installer', 'XYZ_GAME_APP_INSTALLER_XYZ'

    # 5. Services & Socials
    $c = $c -replace '\bKo-fi\b', 'XYZ_KO_FI_XYZ'
    $c = $c -replace 'Discord Webhook', 'XYZ_DISCORD_WEBHOOK_XYZ'
    $c = $c -replace '\bWebhook\b', 'XYZ_WEBHOOK_XYZ'

    # 6. Packs & Databases
    $c = $c -replace 'PSBBN Art Database', 'XYZ_PSBBN_ART_DB_XYZ'
    $c = $c -replace 'HDD-OSD Icon Database', 'XYZ_HDD_OSD_ICON_DB_XYZ'
    $c = $c -replace 'OPL B-APPS Cover Pack', 'XYZ_OPL_BAPPS_PACK_XYZ'
    $c = $c -replace 'OPL Discs & Boxes Pack', 'XYZ_OPL_DISCS_PACK_XYZ'
    $c = $c -replace 'OPL Manager Art Database', 'XYZ_OPL_MGR_ART_DB_XYZ'
    $c = $c -replace '\bSMB\b', 'XYZ_SMB_TOKEN_XYZ'
    $c = $c -replace '(?i)\breadme(?:\.md)?\b', 'XYZ_README_TOKEN_XYZ'

    # 7. Headings & Special Menus
    $c = $c -replace '\bMain Menu\b', 'XYZ_MAIN_MENU_XYZ'
    $c = $c -replace 'Video demonstration of PSBBN', 'XYZ_VIDEO_DEMO_PSBBN_XYZ'

    return $c
}

function Restore-MarkdownTokens([string]$text, $map) {
    ...
    # 2. Restore Markdown URLs: https://uN.link -> original URL
    if ($map.urls -and $map.urls.Count -gt 0) {
        $c = [regex]::Replace($c, 'https?://u(\d+)\.link', {
            param($m)
            $idx = [int]$m.Groups[1].Value
    ...
    # 3. Restore Raw URLs: https://rN.link -> original URL
    if ($map.raws -and $map.raws.Count -gt 0) {
        $c = [regex]::Replace($c, 'https?://r(\d+)\.link', {
            param($m)
            $idx = [int]$m.Groups[1].Value
            if ($idx -lt $map.raws.Count) { return $map.raws[$idx] }
            return $m.Value
        })
    }
    ...
    if ($map.urls -and $map.urls.Count -gt 0) {
        ...
        $c = [regex]::Replace($c, '(?i)\(\s*(?:XYZ)?\s*(?:MD|XMD|XYX|YMD)?\s*URL\D*(\d+)[^)]*\)', {
            param($m)
            $idx = [int]$m.Groups[1].Value
            if ($idx -lt $map.urls.Count) { return "(" + $map.urls[$idx] + ")" }
            return $m.Value
        })
        $c = [regex]::Replace($c, '(?i)(?:XYZ)?\s*(?:MD|XMD|XYX|YMD)\s*URL\D*(\d+)\D*(?:XYZ)?', {
            param($m)
            $idx = [int]$m.Groups[1].Value
            if ($idx -lt $map.urls.Count) { return $map.urls[$idx] }
            return $m.Value
...
function Format-TitleCase([string]$str, [string]$langCode = "pt") {
    ...
    $stopWords = @(
        'de', 'do', 'da', 'dos', 'das', 'em', 'no', 'na', 'nos', 'nas', 'e', 'ou', 'um', 'uma', 'uns', 'umas', 'para', 'por', 'com', 'sem', 'a', 'o', 'os', 'as', 'ao', 'aos', 'Ã ', 'Ã s',
        ...
        'de', 'du', 'des', 'le', 'la', 'les', 'un', 'une', 'et', 'ou', 'Ã ', 'en', 'dans', 'par', 'pour', 'sur',
        'der', 'die', 'das', 'den', 'dem', 'des', 'ein', 'eine', 'eines', 'einem', 'einen', 'einer', 'und', 'oder', 'in', 'an', 'auf', 'fÃ¼r', 'mit', 'von', 'zu',
        'ng', 'mga', 'sa', 'at', 'o', 'ni', 'kay', 'ay', 'nang', 'na',
...
function Invoke-ModuleReadme($targetLangObj, $noWait = $false) {
    $ui = Get-ScriptUI
    try { $Host.UI.RawUI.WindowTitle = $ui.ReadmeTitle } catch { }

    $baseDir = Join-Path $ScriptDir "Readme"
    ...
    $sourceFile = Join-Path $inputDir "README.md"
    $onlineUrl = "https://raw.githubusercontent.com/CosmicScale/PSBBN-Definitive-Project/refs/heads/main/README.md"
    ...
    if (-not (Test-Path $sourceFile)) {
        $localCandidates = @(
            (Join-Path $ScriptDir "_Docs\_PSBBN Language update\README.md"),
            (Join-Path $ScriptDir "_PSBBN Language update\README.md"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_Docs\_PSBBN Language update\README.md"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_PSBBN Language update\README.md")
        )
    ...
    if (-not (Test-Path $sourceFile)) {
        ...
        Write-Host ("  " + $ui.ReadmeNotFoundTitle) -ForegroundColor Red
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + ($ui.ExpectedPath -f $sourceFile)) -ForegroundColor Yellow
        Write-Host "    -> $sourceFile" -ForegroundColor White
        Write-Host ""
        Write-Host ("  " + $ui.ReadmeEnsure) -ForegroundColor DarkYellow
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ""
    ...
    $outFileName = if ($targetLangObj.code -eq 'pt') { "README-PT-BR.md" } elseif ($targetLangObj.code -eq 'pt-pt') { "README-PT-PT.md" } else { "README-{0}.md" -f $targetLangObj.code3.ToUpper() }
    $destFile = Join-Path $outputDir $outFileName
    $cache = @{}

    Write-Host ""
    Show-HeaderBanner "PSBBN README TRANSLATOR"
    ...
    $batchList     = New-Object System.Collections.Generic.List[string]
    $batchIndices  = New-Object System.Collections.Generic.List[int]
    $batchPrefixes = New-Object System.Collections.Generic.List[string]
    $batchMaps     = New-Object System.Collections.Generic.List[object]

    $flushBatch = {
        if ($batchList.Count -eq 0) { return }
        $joined = $batchList -join " ~|~ "
        $translated = Invoke-SingleTranslation $joined $targetLangObj.google
        if (Is-ValidTranslation $translated) {
            $parts = $translated -split '\s*~\|~\s*'
            for ($k = 0; $k -lt $batchList.Count; $k++) {
                $orig = $batchList[$k]
                $trans = if ($k -lt $parts.Length) { $parts[$k] } else { $orig }
                $map = $batchMaps[$k]
                $restored = Restore-MarkdownTokens $trans $map
                $polished = Apply-TermReplacements $restored $targetLangObj $false
                if (Is-ValidTranslation $polished) {
                    $cache[$orig] = $polished
                }
                $translatedLines[$batchIndices[$k]] = "$($batchPrefixes[$k])$polished"
            }
        }
        $batchList.Clear()
        $batchIndices.Clear()
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        $line = $lines[$i]
        Show-InlineProgress $ui.Translating ($i + 1) $total

        # Check code fence boundaries (``` or ~~~)
        if ($line -match '^\s*(```|~~~)') {
            & $flushBatch
        ...
        # 0. Inside Code Fences: Keep 100% UNTOUCHED in raw English
        if ($inCodeFence) {
            & $flushBatch
            $translatedLines.Add($line)
            continue
        ...
        # 1b. README Language Navigation Bar: Keep native language names intact
        if ($line -match '^\s*\|\s*(\*\*|\[)?English(\*\*|\]|\)).*Portugu') {
            & $flushBatch
            if ($targetLangObj.code -eq 'pt') {
                $translatedLines.Add('| [English](https://github.com/CosmicScale/PSBBN-Definitive-Project/blob/main/README.md) | **Português (Brasil)** |')
            } elseif ($targetLangObj.code -eq 'pt-pt') {
                $translatedLines.Add('| [English](https://github.com/CosmicScale/PSBBN-Definitive-Project/blob/main/README.md) | **Português (Portugal)** |')
            } else {
                $translatedLines.Add('| **English** | [Português (Brasil)](https://github.com/CosmicScale/PSBBN-Definitive-Project/blob/main/README-PT-BR.md) |')
            }
            continue
        }

        # 1c. Direct full-line cache match
        if ($cache.ContainsKey($line) -and (Is-ValidTranslation $cache[$line])) {
            & $flushBatch
            $polished = Apply-TermReplacements $cache[$line] $targetLangObj $false
            $translatedLines.Add($polished)
            continue
        }

        # 2. Video Badges: [![Alt](img)](link)
        if ($line -match '^\s*\[\!\[(.*?)\]\((.*?)\)\]\((.*?)\)(\s*)$') {
            & $flushBatch
            $altText = $matches[1].Trim()
            $imgUrl = $matches[2].Trim()
            $linkUrl = $matches[3].Trim()
            $trailing = $matches[4]

            if ($altText -match '(?i)License') {
                $translatedLines.Add($line)
                continue
            }

            $tokenMap = $null
            $shText = Tokenize-MarkdownTokens $altText ([ref]$tokenMap)
            $shText = Protect-TermsBeforeTranslation $shText

            $transAlt = if ($cache.ContainsKey($altText) -and (Is-ValidTranslation $cache[$altText])) {
                $cache[$altText]
            } else {
                $r = Invoke-SingleTranslation $shText $targetLangObj.google
                if (Is-ValidTranslation $r) { $cache[$altText] = $r; $r } else { $altText }
            }
            $restoredAlt = Restore-MarkdownTokens $transAlt $tokenMap
            $polishedAlt = Apply-TermReplacements $restoredAlt $targetLangObj $false
            $titledAlt = Format-TitleCase $polishedAlt $targetLangObj.code

            $rebuilt = "[![{0}]({1})]({2}){3}" -f $titledAlt, $imgUrl, $linkUrl, $trailing
            $translatedLines.Add($rebuilt)
            continue
        }

        # 3. Markdown Headings (# Header Text)
        if ($line -match '^(#{1,6})\s*(.*)$') {
            & $flushBatch
            $level = $matches[1]
            $headingText = $matches[2].Trim()

            # Special Headings (Main Menu, Video Demo, Credits, etc.)
            if ($headingText -match '(?i)\bMain\s+Menu\b') {
                $transH = switch ($targetLangObj.code) {
                    { $_ -in @("pt", "pt-pt") } { "Menu Principal" }
                    "es" { [regex]::Unescape("Men\u00FA Principal") }
                    "fr" { "Menu Principal" }
                    "it" { "Menu Principale" }
                ...
                $translatedLines.Add("$level $transH")
                continue
            }
            if ($headingText -match '(?i)Video\s+demonstration\s+of\s+PSBBN') {
                $transH = switch ($targetLangObj.code) {
                    { $_ -in @("pt", "pt-pt") } { [regex]::Unescape("Demonstra\u00E7\u00E3o em V\u00EDdeo do PSBBN") }
                    "es" { [regex]::Unescape("Demostraci\u00F3n en Video de PSBBN") }
                    "fr" { [regex]::Unescape("D\u00E9monstration Vid\u00E9o de PSBBN") }
            ...
            $tokenMap = $null
            $shHeading = Tokenize-MarkdownTokens $headingText ([ref]$tokenMap)
            $shHeading = Protect-TermsBeforeTranslation $shHeading

            $transH = if ($cache.ContainsKey($headingText) -and (Is-ValidTranslation $cache[$headingText])) {
                $cache[$headingText]
            } else {
                $r = Invoke-SingleTranslation $shHeading $targetLangObj.google
                if (Is-ValidTranslation $r) { $cache[$headingText] = $r; $r } else { $headingText }
            }
            $restoredH = Restore-MarkdownTokens $transH $tokenMap
            $polishedH = Apply-TermReplacements $restoredH $targetLangObj $false
            $titledH = Format-TitleCase $polishedH $targetLangObj.code

            $translatedLines.Add("$level $titledH")
            continue
        }

        # 4. Standalone Markdown Links: [Text](URL)
        if ($line -match '^\s*\[(.*?)\]\((.*?)\)(\s*)$') {
            & $flushBatch
            $linkText = $matches[1]
            $anchor = $matches[2]
            ...
            $tokenMap = $null
            $shLink = Tokenize-MarkdownTokens $linkText ([ref]$tokenMap)
            $shLink = Protect-TermsBeforeTranslation $shLink

            $transL = if ($cache.ContainsKey($linkText) -and (Is-ValidTranslation $cache[$linkText])) {
                $cache[$linkText]
            } else {
                $r = Invoke-SingleTranslation $shLink $targetLangObj.google
                if (Is-ValidTranslation $r) { $cache[$linkText] = $r; $r } else { $linkText }
            }
            $restoredL = Restore-MarkdownTokens $transL $tokenMap
            $polishedL = Apply-TermReplacements $restoredL $targetLangObj $false
            $titledL = Format-TitleCase $polishedL $targetLangObj.code

            $translatedLines.Add(("[{0}]({1}){2}" -f $titledL, $anchor, $trailing))
            continue
        }

        # 5. Bold Markdown Links: **[Text](URL)**
        if ($line -match '^\s*\*\*\[(.*?)\]\((.*?)\)\*\*(\s*)$') {
            & $flushBatch
            $linkText = $matches[1]
            $anchor = $matches[2]
        ...
        # 5b. Regular Bold Title lines: **Title:**
        if ($line -match '^\s*\*\*(.*?)\*\*(\s*)$') {
            & $flushBatch
            $boldContent = $matches[1]
            $trailingB = $matches[2]

            $tokenMap = $null
            $shBold = Tokenize-MarkdownTokens $boldContent ([ref]$tokenMap)
            $shBold = Protect-TermsBeforeTranslation $shBold

            $transB = if ($cache.ContainsKey($boldContent) -and (Is-ValidTranslation $cache[$boldContent])) {
                $cache[$boldContent]
            } else {
                $r = Invoke-SingleTranslation $shBold $targetLangObj.google
                if (Is-ValidTranslation $r) { $cache[$boldContent] = $r; $r } else { $boldContent }
            }
            $restoredB = Restore-MarkdownTokens $transB $tokenMap
            $polishedB = Apply-TermReplacements $restoredB $targetLangObj $false
            $titledB = Format-TitleCase $polishedB $targetLangObj.code

            $translatedLines.Add(("**{0}**{1}" -f $titledB, $trailingB))
            continue
        }

        # 6. General Content (Batching + Cache with list marker preservation)
        $listPrefix = ""
        $rawContent = $line
        if ($line -match '^(\s*[-*]\s+)(.*)$') {
        ...
        if ($cache.ContainsKey($rawContent) -and (Is-ValidTranslation $cache[$rawContent])) {
            $polished = Apply-TermReplacements $cache[$rawContent] $targetLangObj $false
            $translatedLines.Add("$listPrefix$polished")
        } else {
            $tokenMap = $null
            $shContent = Tokenize-MarkdownTokens $rawContent ([ref]$tokenMap)
            $shContent = Protect-TermsBeforeTranslation $shContent

            # If line has any markdown links, translate individually to guarantee zero cross-talk and zero delimiter merging
            if ($tokenMap -and $tokenMap.urls -and $tokenMap.urls.Count -gt 0) {
                & $flushBatch
                $r = Invoke-SingleTranslation $shContent $targetLangObj.google
                $transLine = if (Is-ValidTranslation $r) { $r } else { $shContent }
                $restored = Restore-MarkdownTokens $transLine $tokenMap
                $polished = Apply-TermReplacements $restored $targetLangObj $false
                if (Is-ValidTranslation $polished) {
                    $cache[$rawContent] = $polished
                }
                $translatedLines.Add("$listPrefix$polished")
                continue
            }

            $batchList.Add($shContent)
            $batchIndices.Add($translatedLines.Count)
            $batchPrefixes.Add($listPrefix)
            $batchMaps.Add($tokenMap)
            $translatedLines.Add($shContent) # placeholder

            if ($batchList.Count -ge 15 -or $i -eq ($lines.Count - 1)) {
                & $flushBatch
            }
        }
    }
    ...
    # ----------------------------------------------------------------------
    # Dynamic Markdown Anchor Slug Translation (maps (#english-slug) to (#translated-slug))
    # ----------------------------------------------------------------------
    $slugMap = @{}
    for ($idx = 0; $idx -lt $lines.Count; $idx++) {
        if ($lines[$idx] -match '^(#{1,6})\s+(.*)$') {
            $origHeading = $matches[2].Trim()
            $origSlug = ('#' + (Get-GithubSlug $origHeading)).ToLower()
            if ($idx -lt $translatedLines.Count -and $translatedLines[$idx] -match '^(#{1,6})\s*(.*)$') {
                $transHeading = $matches[2].Trim()
                $transSlug = ('#' + (Get-GithubSlug $transHeading)).ToLower()
                if ($origSlug -ne '#' -and $transSlug -ne '#') {
                    $slugMap[$origSlug] = $transSlug
                }
            }
        }
    }
    # Known aliases / secondary anchors
    if ($slugMap.ContainsKey('#install-games-and-apps')) {
        $slugMap['#game-and-app-installer'] = $slugMap['#install-games-and-apps']
    }
    if ($slugMap.ContainsKey('#clear-art-icon-cache')) {
        $slugMap['#clear-art--icon-cache'] = $slugMap['#clear-art-icon-cache']
    }

    # Second pass: replace anchors in all translated lines (case-insensitive lookup)
    for ($idx = 0; $idx -lt $translatedLines.Count; $idx++) {
        $tLine = $translatedLines[$idx]
        if ($tLine -match '\(\*{0,2}#[a-zA-Z0-9\-_]+\*{0,2}\)') {
            $translatedLines[$idx] = [regex]::Replace($tLine, '\(\*{0,2}(#[a-zA-Z0-9\-_]+)\*{0,2}\)', {
                param($m)
                $raw = $m.Groups[1].Value.Trim()
                $a = $raw.ToLower()
                if ($slugMap.ContainsKey($a)) {
                    return '(' + $slugMap[$a] + ')'
                }
                return '(' + $a + ')'
            })
    ...
    Write-Host ""
        # Safety sweep: recover any link tokens that survived using original file links
    for ($idx = 0; $idx -lt $translatedLines.Count; $idx++) {
        $tLine = $translatedLines[$idx]
        if ($tLine -match 'https?://[ur]\d+\.link' -and $idx -lt $lines.Count) {
            $orig = $lines[$idx]
            $origUrls = New-Object System.Collections.Generic.List[string]
            $mUrls = [regex]::Matches($orig, '\]\((https?://[^)\s]+)\)')
            foreach ($mu in $mUrls) { $origUrls.Add($mu.Groups[1].Value) }
            $translatedLines[$idx] = [regex]::Replace($tLine, 'https?://u(\d+)\.link', {
                param($m)
                $uIdx = [int]$m.Groups[1].Value
                if ($uIdx -lt $origUrls.Count) { return $origUrls[$uIdx] }
                return $m.Value
            })
        }
        $translatedLines[$idx] = Apply-TermReplacements $translatedLines[$idx] $targetLangObj $false
    }
    ...
    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.CompletionBanner -f "Readme")) -ForegroundColor Green
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ""
...
function Invoke-ModuleChangelogMain($targetLangObj, $noWait = $false) {
    $ui = Get-ScriptUI
    try { $Host.UI.RawUI.WindowTitle = $ui.ChangelogMainTitle } catch { }

    $baseDir = Join-Path $ScriptDir "Changelog_Main"
    $inputDir = Join-Path $baseDir "input"
    $outputDir = Join-Path $baseDir "output"
    foreach ($d in @($inputDir, $outputDir)) {
        if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d -Force | Out-Null }
    ...
    $sourceFile = Join-Path $inputDir "changelog_main_eng.txt"
    $onlineUrl = "https://raw.githubusercontent.com/CosmicScale/PSBBN-Definitive-Project/main/scripts/assets/lang/changelog_main_eng.txt"
    ...
    if (-not (Test-Path $sourceFile)) {
        $localCandidates = @(
            (Join-Path $ScriptDir "_Docs\_PSBBN Language update\changelog_main_eng.txt"),
            (Join-Path $ScriptDir "_PSBBN Language update\changelog_main_eng.txt"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_Docs\_PSBBN Language update\changelog_main_eng.txt"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_PSBBN Language update\changelog_main_eng.txt")
        )
        foreach ($cand in $localCandidates) {
            if (Test-Path $cand) {
                try { Copy-Item -Path $cand -Destination $sourceFile -Force | Out-Null; break } catch { }
    ...
    if (-not (Test-Path $sourceFile)) {
        Write-Host ""
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + $ui.ChangelogMainNotFoundTitle) -ForegroundColor Red
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + ($ui.ExpectedPath -f $sourceFile)) -ForegroundColor Yellow
        Write-Host "    -> $sourceFile" -ForegroundColor White
        Write-Host ""
        Write-Host ("  " + $ui.GenericEnsureInternetOrInput) -ForegroundColor DarkYellow
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ""
        Write-Host ("  " + $ui.PressAnyKey) -ForegroundColor Cyan
        [void][Console]::ReadKey($true)
    ...
    $codeSuffix = if ($targetLangObj.code -eq 'pt-pt') { "por-pt" } else { $targetLangObj.code3.ToLower() }
    $outFileName = ("changelog_main_{0}.txt" -f $codeSuffix)
    $destFile = Join-Path $outputDir $outFileName
    $cache = @{}

    Write-Host ""
    Show-HeaderBanner "PSBBN CHANGELOG MAIN TRANSLATOR"
    Write-Host ("  {0} {1} ({2})" -f $ui.Translating, $targetLangObj.name, $targetLangObj.native) -ForegroundColor White
    Write-Host ("  {0} {1}" -f $ui.SourceFile, $sourceFile) -ForegroundColor DarkGray
    Write-Host ("  {0} {1}" -f $ui.DestFile, $destFile) -ForegroundColor DarkGray
    Write-Host ""
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if ([string]::IsNullOrWhiteSpace($line) -or $line -match '^[0-9.,;:\s+/%-]+$') {
            $translatedLines.Add($line)
            continue
        }

        if ($cache.ContainsKey($line) -and (Is-ValidTranslation $cache[$line])) {
            $polished = Apply-TermReplacements $cache[$line] $targetLangObj $true
            $translatedLines.Add($polished)
        } else {
            # Protect PSBBN Definitive Project and Channel Brands in Changelogs
            $tokenized = $line -replace 'PSBBN Definitive Project', 'XYZPSBBNDEFPROJXYZ'
            $tokenized = $tokenized -replace '(?i)\b(?:The\s+)?Game Channel\b', 'XYZGAMECHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\b(?:The\s+)?Internet Channel\b', 'XYZINTERNETCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\b(?:The\s+)?Movie Channel\b', 'XYZMOVIECHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\b(?:The\s+)?Music Channel\b', 'XYZMUSICCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\b(?:The\s+)?Photo Channel\b', 'XYZPHOTOCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bBANDAI CHANNEL\b', 'XYZBANDAICHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bKONAMI CHANNEL\b', 'XYZKONAMICHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bCAPCOM CHANNEL\b', 'XYZCAPCOMCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bNAMCO CHANNEL\b', 'XYZNAMCOCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bHUDSON CHANNEL\b', 'XYZHUDSONCHANNELXYZ'
            $tokenized = $tokenized -replace '(?i)\bEA CHANNEL\b', 'XYZEACHANNELXYZ'

            $batchList.Add($tokenized)
            $batchIndices.Add($translatedLines.Count)
            $translatedLines.Add($tokenized)

            if ($batchList.Count -ge 15 -or $i -eq ($lines.Count - 1)) {
                [string[]]$batchRes = @(Invoke-BatchTranslation $batchList.ToArray() $targetLangObj.google)
                for ($k = 0; $k -lt $batchList.Count; $k++) {
                    $orig = $lines[$batchIndices[$k]]
                    $trans = if ($k -lt $batchRes.Count -and $batchRes[$k] -and (Is-ValidTranslation [string]$batchRes[$k])) { [string]$batchRes[$k] } else { $orig }
                    if ($trans.Length -eq 1 -and $orig.Length -gt 2) {
                        $trans = $orig
                    }
                    $polished = Apply-TermReplacements $trans $targetLangObj $true
                    if (Is-ValidTranslation $polished) {
                        $cache[$orig] = $polished
                    }
                    $translatedLines[$batchIndices[$k]] = $polished
                }
                $batchList.Clear()
                $batchIndices.Clear()
            }
    ...
    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.CompletionBanner -f "Changelog Main")) -ForegroundColor Green
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ""
    if (-not $noWait) { return (Wait-EndNavigation -inputFiles @($sourceFile)) }
    return "menu"
...
function Invoke-ModuleChangelogPatch($targetLangObj, $noWait = $false) {
    $ui = Get-ScriptUI
    try { $Host.UI.RawUI.WindowTitle = $ui.ChangelogPatchTitle } catch { }

    $baseDir = Join-Path $ScriptDir "Changelog_Patch"
    $inputDir = Join-Path $baseDir "input"
    $outputDir = Join-Path $baseDir "output"
    foreach ($d in @($inputDir, $outputDir)) {
        if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d -Force | Out-Null }
    ...
    $sourceFile = Join-Path $inputDir "changelog_patch_eng.txt"
    $onlineUrl = "https://raw.githubusercontent.com/CosmicScale/PSBBN-Definitive-Project/main/scripts/assets/lang/changelog_patch_eng.txt"
    ...
    if (-not (Test-Path $sourceFile)) {
        $localCandidates = @(
            (Join-Path $ScriptDir "_Docs\_PSBBN Language update\changelog_patch_eng.txt"),
            (Join-Path $ScriptDir "_PSBBN Language update\changelog_patch_eng.txt"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_Docs\_PSBBN Language update\changelog_patch_eng.txt"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_PSBBN Language update\changelog_patch_eng.txt")
        )
        foreach ($cand in $localCandidates) {
            if (Test-Path $cand) {
                try { Copy-Item -Path $cand -Destination $sourceFile -Force | Out-Null; break } catch { }
    ...
    if (-not (Test-Path $sourceFile)) {
        Write-Host ""
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + $ui.ChangelogPatchNotFoundTitle) -ForegroundColor Red
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + ($ui.ExpectedPath -f $sourceFile)) -ForegroundColor Yellow
        Write-Host "    -> $sourceFile" -ForegroundColor White
        Write-Host ""
    ...
    $codeSuffix = if ($targetLangObj.code -eq 'pt-pt') { "por-pt" } else { $targetLangObj.code3.ToLower() }
    $outFileName = ("changelog_patch_{0}.txt" -f $codeSuffix)
    $destFile = Join-Path $outputDir $outFileName
    $cache = @{}

    Write-Host ""
    Show-HeaderBanner "PSBBN CHANGELOG PATCH TRANSLATOR"
    Write-Host ("  {0} {1} ({2})" -f $ui.Translating, $targetLangObj.name, $targetLangObj.native) -ForegroundColor White
    Write-Host ("  {0} {1}" -f $ui.SourceFile, $sourceFile) -ForegroundColor DarkGray
    Write-Host ("  {0} {1}" -f $ui.DestFile, $destFile) -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.CompletionBanner -f "Changelog Patch")) -ForegroundColor Green
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ""
    if (-not $noWait) { return (Wait-EndNavigation -inputFiles @($sourceFile)) }
    return "menu"
...
function Repair-PsbbnXml($rawContent) {
    if ([string]::IsNullOrWhiteSpace($rawContent)) { return $rawContent }
    $c = $rawContent

    # 1. Auto-close unclosed <LINE> tags (e.g. kanarazu_C_8.xml line 46)
    $c = [regex]::Replace($c, '(?mi)^(\s*<line\b[^>]*>(?!.*?</(?:line|LINE)>)[^\r\n]*?)\s*$', '$1</LINE>')

    # 2. Fix known unescaped quotes in error.xml line 13 ("Check/Change") safely without affecting other attributes
    $c = $c -replace '(?i)"(Check/Change)"', '&quot;$1&quot;'

    # 3. Escape raw unescaped & inside text or attributes (e.g. guide_D_60.xml)
    $c = [regex]::Replace($c, '&(?!(?:amp|lt|gt|quot|apos|#\d+|#x[0-9a-fA-F]+);)', '&amp;')

    # 4. Normalize single-quoted attributes to double-quoted attributes
    # Convert name='val' to name="val" (escaping any internal double quotes to &quot;)
    # Guarantees standard XML attributes and prevents parse errors when values have natural apostrophes (e.g. Italian/French elisions)
    $c = [regex]::Replace($c, '(\b[a-zA-Z_:][a-zA-Z0-9_\.:-]*\s*=\s*)''([^'']*)''', {
        param($m)
...
function Format-PsbbnGuideParagraph($text, $maxLen = 52, $indent = "", [bool]$isBullet = $false) {
    if ([string]::IsNullOrWhiteSpace($text)) { return "`t<LINE></LINE>" }
    $words = ($text.Trim()) -split '\s+'
    $lines = New-Object System.Collections.Generic.List[string]
    $curr = $indent
    $hangingIndent = if ($isBullet -or $text -match '^[\u30FB\u2022\u00B7]') { "  " } else { $indent }
    foreach ($w in $words) {
        if ($curr -eq $indent) {
            $curr += $w
        } elseif (($curr.Length + 1 + $w.Length) -le $maxLen) {
            $curr += " " + $w
        } else {
            $lines.Add($curr)
            $curr = "$hangingIndent$w"
        }
    }
    if ($curr -ne $indent -and $curr -ne $hangingIndent) { $lines.Add($curr) }
    return ($lines | ForEach-Object { "`t<LINE>$_</LINE>" }) -join "`r`n"
}

function Parse-PsbbnTextareaBlocks([string]$innerXml) {
    $lines = [regex]::Matches($innerXml, '(?i)<LINE\b[^>]*>(.*?)</LINE>') | ForEach-Object { $_.Groups[1].Value }
    ...
    $flush = {
        if (-not [string]::IsNullOrEmpty($currPara)) {
            $blocks.Add([PSCustomObject]@{ Type = $currType; Text = $currPara; Indent = $currIndent })
            $currPara = ""
            $currIndent = ""
            $currType = "Para"
        }
    ...
    for ($idx = 0; $idx -lt $lines.Count; $idx++) {
        ...
        if ([string]::IsNullOrWhiteSpace($l)) {
            . $flush
            $blocks.Add([PSCustomObject]@{ Type = "Empty"; Text = ""; Indent = "" })
            continue
        }

        $trimmed = $l.Trim()

        # Pure tag lines or colored headers (e.g. @@IMG ...@@ or @@FONT ...@@【Tips】)
        if ($trimmed -match '^@@IMG\b' -or $trimmed -match '^@@FONT\b.*@@[\u3010\u25C6\[]') {
            . $flush
            $blocks.Add([PSCustomObject]@{ Type = "Header"; Text = $trimmed; Indent = "" })
            continue
        }

        # Section headers: ◆Title◆ or 【Title】
        if ($trimmed -match '^[\u25C6\u3010].*?[\u25C6\u3011]$' -or ($trimmed -match '^[\u25C6\u3010]' -and $trimmed.Length -le 35)) {
            . $flush
            $blocks.Add([PSCustomObject]@{ Type = "Header"; Text = $trimmed; Indent = "" })
            continue
        }

        # Bullet titles vs bullet multi-line sentences (\u30FB = ・, \u2022 = •, \u00B7 = ·)
        if ($trimmed -match '^([\u30FB\u2022\u00B7]|\.\s+)') {
            . $flush
            $isShortTitle = ($trimmed.Length -le 25)
            $nextIsContinuation = $false
            if ($idx + 1 -lt $lines.Count) {
                $nextL = $lines[$idx + 1].Trim()
                if ($nextL -cmatch '^[a-z]' -or ($trimmed -notmatch '[.:?!]$' -and $trimmed.Length -gt 35)) {
                    $nextIsContinuation = $true
                }
            }

            if ($isShortTitle -and -not $nextIsContinuation) {
                $blocks.Add([PSCustomObject]@{ Type = "Header"; Text = $trimmed; Indent = "" })
                continue
            } else {
                $currPara = $trimmed
        ...
        # Regular body line
        $indentMatch = [regex]::Match($l, '^(\s*)')
        $indent = $indentMatch.Groups[1].Value

        if ([string]::IsNullOrEmpty($currPara)) {
            $currPara = $trimmed
            $currIndent = $indent
            $currType = "Para"
        } else {
...
function Set-TarExecutablePermissions([string]$tarPath) {
    if (-not (Test-Path $tarPath)) { return }
    $bytes = [System.IO.File]::ReadAllBytes($tarPath)
    $pos = 0
    $patchedCount = 0
    while ($pos + 512 -le $bytes.Length) {
        $name = [System.Text.Encoding]::ASCII.GetString($bytes, $pos, 100).Trim([char]0)
        if ([string]::IsNullOrWhiteSpace($name)) { break }

        $sizeOctal = [System.Text.Encoding]::ASCII.GetString($bytes, $pos + 124, 12).Trim([char]0, ' ')
        $size = if ($sizeOctal) { [Convert]::ToInt64($sizeOctal, 8) } else { 0 }

        # Identify binaries, scripts, or the 'bn' executable itself
        if ($name -match '(^|/)bn$' -or $name -match '\.(sh|elf|bin)$') {
            # Set POSIX mode to 0755 (rwxr-xr-x): "0000755\0"
            $newMode = [System.Text.Encoding]::ASCII.GetBytes("0000755`0")
            [Array]::Copy($newMode, 0, $bytes, $pos + 100, 8)

            # Blank out checksum field with spaces
            for ($c = 148; $c -lt 156; $c++) { $bytes[$pos + $c] = 32 }

            $sum = 0
            for ($c = 0; $c -lt 512; $c++) { $sum += [int]$bytes[$pos + $c] }

            $chkStr = ([Convert]::ToString($sum, 8)).PadLeft(6, '0') + "`0 "
            $chkBytes = [System.Text.Encoding]::ASCII.GetBytes($chkStr)
            [Array]::Copy($chkBytes, 0, $bytes, $pos + 148, 8)
            $patchedCount++
        }
    ...
    [System.IO.File]::WriteAllBytes($tarPath, $bytes)
        $ui = Get-ScriptUI; Write-Host ("  " + ($ui.PosixPatchedCount -f $patchedCount)) -ForegroundColor Green
}

function Format-PsbbnGuideLine($text, $maxLen = 56) {
    if ([string]::IsNullOrWhiteSpace($text) -or $text.Length -le $maxLen) {
    ...
    if (-not [string]::IsNullOrEmpty($curr)) { $lines.Add($curr) }
    $out = ($lines | ForEach-Object { "<LINE>$_</LINE>" }) -join "`r`n`t"
    return $out
}

function Adjust-PsbbnTextareaHeader([string]$xmlContent) {
    if ([string]::IsNullOrWhiteSpace($xmlContent)) { return $xmlContent }
    return [regex]::Replace($xmlContent, '(?i)(<TEXTAREA\b[^>]*>)([\s\S]*?)(</TEXTAREA>)', {
        param($m)
        $openTag = $m.Groups[1].Value
        $inner = $m.Groups[2].Value
        $closeTag = $m.Groups[3].Value

        $lineCount = ([regex]::Matches($inner, '(?i)<line\b')).Count
        if ($lineCount -gt 0) {
            $newVisible = if ($lineCount -gt 14) { "14" } else { [string]$lineCount }
            $newScrollbar = if ($lineCount -gt 14) { "true" } else { "false" }

            if ($openTag -match '(?i)\bvisible=') {
                $openTag = [regex]::Replace($openTag, '(?i)\bvisible=(["''])[0-9]+(["''])', "visible=`"$newVisible`"")
            } else {
                $openTag = $openTag -replace '>$', " visible=`"$newVisible`">"
            }

            if ($openTag -match '(?i)\bscrollbar=') {
                $openTag = [regex]::Replace($openTag, '(?i)\bscrollbar=(["''])(?:true|false)(["''])', "scrollbar=`"$newScrollbar`"")
            } else {
                $openTag = $openTag -replace '>$', " scrollbar=`"$newScrollbar`">"
            }
        }
...
function Invoke-ModuleSystemPsbbn($targetLangObj, $noWait = $false) {
    $ui = Get-ScriptUI
    try { $Host.UI.RawUI.WindowTitle = $ui.SystemTitle } catch { }

    $baseDir = Join-Path $ScriptDir "System PSBBN"
    $inputDir = Join-Path $baseDir "input"
    $outputDir = Join-Path $baseDir "output"
    foreach ($d in @($inputDir, $outputDir)) {
        if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d -Force | Out-Null }
    ...
    $packOnlineUrl = "https://github.com/user-attachments/files/26068990/PSBBN-translation-pack-v3.5.zip"
    $refDir = if (Test-Path (Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English")) {
                  Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English"
              } elseif (Test-Path (Join-Path $inputDir "PSBBN_English")) {
                  Join-Path $inputDir "PSBBN_English"
              } elseif (Test-Path "D:\Games\Outros\PSBBN-translation-pack-v3.5 (comecei 26-01-2026 e terminei 07-02-2026)\PSBBN_English") {
                  "D:\Games\Outros\PSBBN-translation-pack-v3.5 (comecei 26-01-2026 e terminei 07-02-2026)\PSBBN_English"
              } else {
                  Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English"
              }
    ...
    if (-not (Test-Path $refDir)) {
        Write-Host ""
        Write-Host ("  " + $ui.DownloadingOfficial) -ForegroundColor Cyan
        Write-Host "      Link: $packOnlineUrl" -ForegroundColor DarkGray
        $zipTemp = Join-Path $inputDir "PSBBN-translation-pack-v3.5.zip"
        try {
            [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12 -bor [System.Net.SecurityProtocolType]::Tls11 -bor [System.Net.SecurityProtocolType]::Tls
            $wc = New-Object System.Net.WebClient
            $wc.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64)")
            $wc.DownloadFile($packOnlineUrl, $zipTemp)
            Write-Host ("  " + ($ui.OfficialDownloadSuccess -f (Get-Item $zipTemp).Length)) -ForegroundColor Green
            $extractMsg = if ($Global:CurrentUILang -eq "en") { "[*] Extracting PSBBN v3.5 translation pack..." } else { "[*] Extraindo pacote de tradução PSBBN v3.5..." }
            Write-Host ("  " + $extractMsg) -ForegroundColor Cyan
            Expand-Archive -Path $zipTemp -DestinationPath $inputDir -Force
            Remove-Item $zipTemp -Force -ErrorAction SilentlyContinue
        } catch {
            Write-Host ("  " + ($ui.CannotConnectGithub -f $_.Exception.Message)) -ForegroundColor Yellow
            Write-Host ("      " + ($ui.UsingFallback -f $refDir)) -ForegroundColor DarkYellow
            Remove-Item $zipTemp -Force -ErrorAction SilentlyContinue
            if (Test-Path "D:\Games\Outros\PSBBN-translation-pack-v3.5 (comecei 26-01-2026 e terminei 07-02-2026)\PSBBN_English") {
                try {
                    $destAuto = Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English"
                    New-Item -ItemType Directory -Path $destAuto -Force | Out-Null
                    Copy-Item -Path "D:\Games\Outros\PSBBN-translation-pack-v3.5 (comecei 26-01-2026 e terminei 07-02-2026)\PSBBN_English\*" -Destination $destAuto -Recurse -Force | Out-Null
                } catch { }
            }
        }
        if (Test-Path (Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English")) {
            $refDir = Join-Path $inputDir "PSBBN-translation-pack-v3.5\PSBBN_English"
        } elseif (Test-Path (Join-Path $inputDir "PSBBN_English")) {
            $refDir = Join-Path $inputDir "PSBBN_English"
        }
    }
    ...
    if (-not (Test-Path $refDir)) {
        Write-Host ""
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + $ui.PsbbnEnglishNotFoundTitle) -ForegroundColor Red
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + ($ui.ExpectedPath -f $sourceFile)) -ForegroundColor Yellow
        Write-Host "    -> $refDir" -ForegroundColor White
        Write-Host ""
        Write-Host ("  " + $ui.PsbbnEnglishEnsure) -ForegroundColor DarkYellow
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ""
        Write-Host ("  " + $ui.PressAnyKey) -ForegroundColor Cyan
        [void][Console]::ReadKey($true)
    ...
    $langFolder = $targetLangObj.dir
    $destFolder = Join-Path $outputDir $langFolder
    $cache = @{}

    Write-Host ""
    Show-HeaderBanner "SYSTEM PSBBN TRANSLATOR"
    Write-Host ("  {0} {1} ({2})" -f $ui.Translating, $targetLangObj.name, $targetLangObj.native) -ForegroundColor White
    Write-Host ("  {0} {1}" -f $ui.SourceFile, $refDir) -ForegroundColor DarkGray
    Write-Host ("  {0} {1}" -f $ui.DestFile, $destFolder) -ForegroundColor DarkGray
    Write-Host ""

    # Step 1: Copy tree if not already copied
    if (-not (Test-Path (Join-Path $destFolder "opt0"))) {
        Write-Host ("  " + $ui.Step1Copying) -ForegroundColor Cyan
        if (-not (Test-Path $destFolder)) {
            New-Item -ItemType Directory -Path $destFolder -Force | Out-Null
        }
        $corruptedNested = Join-Path $destFolder "PSBBN_English"
        if (Test-Path $corruptedNested) {
            Remove-Item $corruptedNested -Recurse -Force | Out-Null
        }
        Copy-Item -Path (Join-Path $refDir "*") -Destination $destFolder -Recurse -Force | Out-Null
        # Intelligent Pre-Repair for official XML bugs (unclosed tags, quotes in attributes, raw ampersands)
        $rawXmls = Get-ChildItem -Path $destFolder -Recurse -Filter "*.xml"
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
        foreach ($rxf in $rawXmls) {
            $rawC = [System.IO.File]::ReadAllText($rxf.FullName, [System.Text.Encoding]::UTF8)
            $fixedC = Repair-PsbbnXml $rawC
            [System.IO.File]::WriteAllText($rxf.FullName, $fixedC, $utf8NoBom)
        }
    } else {
        $corruptedNested = Join-Path $destFolder "PSBBN_English"
        if (Test-Path $corruptedNested) {
    ...
    # Step 2: Full XML Translation (Extracts labels, <TEXT value="...">, <item value="...">, <line>...</line>, <TEXT>...</TEXT>, buttons)
    $xmlFiles = Get-ChildItem -Path $destFolder -Recurse -Filter "*.xml"
    Write-Host ("  " + ($ui.Step2TranslatingXml -f $xmlFiles.Count)) -ForegroundColor Cyan

    $allXmlStrings = New-Object System.Collections.Generic.HashSet[string]
    $jaCharRegex = '[\u3041-\u3096\u30A1-\u30FA\u4E00-\u9FFF]'

    foreach ($xf in $xmlFiles) {
        $isEdclient = $xf.FullName.Contains("edclient")
        $content = [System.IO.File]::ReadAllText($xf.FullName, [System.Text.Encoding]::UTF8)

        # 1. label="..."
        $matches = [regex]::Matches($content, '(?i)label\s*=\s*(["''])(.*?)\1')
        foreach ($m in $matches) {
            $val = $m.Groups[2].Value.Trim()
            if ($val.Length -gt 1 -and -not $val.StartsWith("file:") -and -not $val.StartsWith("http") -and -not $val.StartsWith("mode=") -and ($val -match '[a-zA-Z]')) {
                if (-not ($isEdclient -and ($val -match $jaCharRegex))) {
                    [void]$allXmlStrings.Add($val)
                }
            }
        }

        # 2. <TEXT ... value="...">, <item ... value="...">, <data ... value="...">, <text_label ... value="...">
        $matches = [regex]::Matches($content, '(?i)<(?:TEXT|item|data|text_label)\s+[^>]*?value\s*=\s*(["''])(.*?)\1')
        foreach ($m in $matches) {
            $val = $m.Groups[2].Value.Trim()
            if ($val.Length -gt 0 -and ($val -match '[a-zA-Z]')) {
                if (-not ($isEdclient -and ($val -match $jaCharRegex))) {
                    [void]$allXmlStrings.Add($val)
                }
            }
        ...
        # 3. <TEXTAREA> blocks (for guides) OR <line>...</line> (for other XMLs)
        if ($content -match '(?i)<TEXTAREA\b[^>]*>([\s\S]*?)</TEXTAREA>') {
            $taInner = $matches[1]
            $taBlocks = Parse-PsbbnTextareaBlocks $taInner
            foreach ($b in $taBlocks) {
                if ($b.Type -eq "Para" -or $b.Type -eq "Header" -or $b.Type -eq "BulletPara") {
                    $val = $b.Text
                    if ($val.Length -gt 0 -and ($val -match '[a-zA-Z]')) {
                        if (-not ($isEdclient -and ($val -match $jaCharRegex))) {
                            [void]$allXmlStrings.Add($val)
            ...
            $matches = [regex]::Matches($content, '(?i)<line>([\s\S]*?)</line>')
            foreach ($m in $matches) {
        ...
        # 4. <TEXT>...</TEXT>
        $matches = [regex]::Matches($content, '(?i)<TEXT(?:\s+[^>]*)?>([^<]+)</TEXT>')
        foreach ($m in $matches) {
            $val = $m.Groups[1].Value.Trim()
            if ($val.Length -gt 0 -and ($val -match '[a-zA-Z]')) {
                if (-not ($isEdclient -and ($val -match $jaCharRegex))) {
        ...
        # 5. Buttons: cross, circle, triangle, close
        $matches = [regex]::Matches($content, '(?i)(?:cross|circle|triangle|close)\s*=\s*(["''])(.*?)\1')
        foreach ($m in $matches) {
            $val = $m.Groups[2].Value.Trim()
            if ($val.Length -gt 0 -and ($val -match '[a-zA-Z]')) {
                if (-not ($isEdclient -and ($val -match $jaCharRegex))) {
    ...
    # Pre-populate cache with PlayStation gaming nuances
    if ($targetLangObj.psConfirm) {
        $cache["Enter"] = $targetLangObj.psConfirm
        $cache["enter"] = $targetLangObj.psConfirm.ToLower()
    }
    if ($targetLangObj.psBack) {
        $cache["Back"] = $targetLangObj.psBack
        $cache["back"] = $targetLangObj.psBack.ToLower()
    }

    $missingStrings = New-Object System.Collections.Generic.List[string]
    foreach ($s in $allXmlStrings) {
        if (-not $cache.ContainsKey($s)) {
            $missingStrings.Add($s)
        }
    }

    if ($missingStrings.Count -gt 0) {
        for ($b = 0; $b -lt $missingStrings.Count; $b += 15) {
            $chunk = @()
            $end = [Math]::Min($missingStrings.Count, $b + 15)
            for ($k = $b; $k -lt $end; $k++) { $chunk += $missingStrings[$k] }
            Show-InlineProgress $ui.ProgressXmlStrings $end $missingStrings.Count

            # Protect PSBBN Definitive Project, PlayStation Now and feega
            $tokenizedChunk = @()
            foreach ($c in $chunk) {
                $t = $c -replace 'PSBBN Definitive Project', 'XYZPSBBNDEFPROJXYZ'
                $t = $t -replace 'PlayStation Now!', 'XYZPSNOWEXCLXYZ'
                $t = $t -replace 'PlayStation Now', 'XYZPSNOWXYZ'
                $t = $t -replace 'FEEGA', 'XYZFEEGAUPPERXYZ'
                $t = $t -replace 'feega', 'XYZFEEGATOKENXYZ'
                $tokenizedChunk += $t
            }

            [string[]]$bRes = @(Invoke-BatchTranslation $tokenizedChunk $targetLangObj.google)
            for ($k = 0; $k -lt $chunk.Count; $k++) {
                $orig = $chunk[$k]
                $trans = if ($k -lt $bRes.Count -and $bRes[$k] -and (Is-ValidTranslation [string]$bRes[$k])) { [string]$bRes[$k] } else { $chunk[$k] }
                if ($trans.Length -eq 1 -and $orig.Length -gt 2) {
                    $trans = $orig
                }
                $polished = Apply-TermReplacements $trans $targetLangObj $false
                if (Is-ValidTranslation $polished) {
                    $cache[$orig] = $polished
                }
            }
            ...
            if (($b % 75) -eq 0) {
                # Translation cache kept strictly in-memory during run; no stale disk persistence
            }
        }
    }

    # Helper function for XML string replacement with entity escaping
    $TranslateXmlVal = {
        param($rawVal, [bool]$isAttribute = $false)
        if ([string]::IsNullOrWhiteSpace($rawVal)) { return $rawVal }
        if ($targetLangObj.psConfirm -and ($rawVal -eq "Enter" -or $rawVal -eq "enter" -or $rawVal -eq "Entrar")) {
            return $targetLangObj.psConfirm
        }
        if ($targetLangObj.psBack -and ($rawVal -eq "Back" -or $rawVal -eq "back")) {
            return $targetLangObj.psBack
        }
        $trans = if ($cache.ContainsKey($rawVal)) {
            Apply-TermReplacements $cache[$rawVal] $targetLangObj $false
        } else {
            Apply-TermReplacements $rawVal $targetLangObj $false
        }
        # Escape raw ampersands
        $trans = [regex]::Replace($trans, '&(?!(?:amp|lt|gt|quot|apos|#\d+|#x[0-9a-fA-F]+);)', '&amp;')
        if ($isAttribute) {
            $trans = $trans -replace '(?<!&quot;)"', '&quot;'
            $trans = $trans -replace '<', '&lt;'
        }
    ...
    # Now apply the translations back to each XML file
    $xmlCounter = 0
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    foreach ($xf in $xmlFiles) {
        $xmlCounter++
        Show-InlineProgress $ui.ProgressUpdatingXml $xmlCounter $xmlFiles.Count

        $isEdclient = $xf.FullName.Contains("edclient")
        $isGuideScene = $xf.FullName.Contains("guide")
        $content = [System.IO.File]::ReadAllText($xf.FullName, [System.Text.Encoding]::UTF8)

        # 1. label="..."
        $content = [regex]::Replace($content, '(?i)(label\s*=\s*)(["''])(.*?)\2', {
            param($m)
            $pre = $m.Groups[1].Value; $val = $m.Groups[3].Value.Trim()
            if ($isEdclient -and ($val -match $jaCharRegex)) { return $m.Value }
            $trans = & $TranslateXmlVal $val $true
            return "$pre`"$trans`""
        })

        # 2. <TEXT ... value="...">, <item ... value="...">, <data ... value="...">, <text_label ... value="...">
        $content = [regex]::Replace($content, '(?i)(<(?:TEXT|item|data|text_label)\s+[^>]*?value\s*=\s*)(["''])(.*?)\2', {
            param($m)
            $pre = $m.Groups[1].Value; $val = $m.Groups[3].Value.Trim()
            if ($isEdclient -and ($val -match $jaCharRegex)) { return $m.Value }
            $trans = & $TranslateXmlVal $val $true
        ...
        # 3. <TEXTAREA> paragraphs (reconstructed & balanced wrapped) OR <line>...</line>
        if ($content -match '(?i)<TEXTAREA\b') {
            $content = [regex]::Replace($content, '(?i)(<TEXTAREA\b[^>]*>)([\s\S]*?)(</TEXTAREA>)', {
                param($m)
                $openTag = $m.Groups[1].Value
                $inner = $m.Groups[2].Value
                $closeTag = $m.Groups[3].Value

                $taBlocks = Parse-PsbbnTextareaBlocks $inner
                $outLines = New-Object System.Collections.Generic.List[string]
                foreach ($b in $taBlocks) {
                    if ($b.Type -eq "Empty") {
                        $outLines.Add("`t<LINE></LINE>")
                    } elseif ($b.Type -eq "Header") {
                        $transH = & $TranslateXmlVal $b.Text $false
                        $outLines.Add("`t<LINE>$transH</LINE>")
                    } elseif ($b.Type -eq "Para") {
                        $transP = & $TranslateXmlVal $b.Text $false
                        $formattedP = Format-PsbbnGuideParagraph $transP 52 $b.Indent $false
                        $outLines.Add($formattedP)
                    } elseif ($b.Type -eq "BulletPara") {
                        $transP = & $TranslateXmlVal $b.Text $false
                        $formattedP = Format-PsbbnGuideParagraph $transP 52 $b.Indent $true
                        $outLines.Add($formattedP)
                    }
                }
                $newInner = "`r`n" + ($outLines -join "`r`n") + "`r`n    "
                return "$openTag$newInner$closeTag"
            })
            $content = Adjust-PsbbnTextareaHeader $content
        } else {
            $content = [regex]::Replace($content, '(?i)(<line>)([\s\S]*?)(</line>)', {
                param($m)
                $pre = $m.Groups[1].Value; $val = $m.Groups[2].Value.Trim(); $post = $m.Groups[3].Value
                if ($isEdclient -and ($val -match $jaCharRegex)) { return $m.Value }
                $trans = & $TranslateXmlVal $val $false
                return "$pre$trans$post"
            })
        }

        # 4. <TEXT>...</TEXT>
        $content = [regex]::Replace($content, '(?i)(<TEXT(?:\s+[^>]*)?>)([^<]+)(</TEXT>)', {
            param($m)
            $pre = $m.Groups[1].Value; $val = $m.Groups[2].Value.Trim(); $post = $m.Groups[3].Value
            if ($isEdclient -and ($val -match $jaCharRegex)) { return $m.Value }
            $trans = & $TranslateXmlVal $val $false
        ...
        # 5. Buttons: cross, circle, triangle, close
        $content = [regex]::Replace($content, '(?i)((?:cross|circle|triangle|close)\s*=\s*)(["''])(.*?)\2', {
            param($m)
            $pre = $m.Groups[1].Value; $val = $m.Groups[3].Value.Trim()
            if ($isEdclient -and ($val -match $jaCharRegex)) { return $m.Value }
            $trans = & $TranslateXmlVal $val $true
        ...
        # 6. Adjust <TEXTAREA> header in guide files according to CosmicScale specification:
        # visible= set to total number of lines or capped at 14 if higher.
        # scrollbar= set to "true" if over 14 lines or false otherwise.
        if ($isGuideScene) {
            $content = Adjust-PsbbnTextareaHeader $content
        }

        # Final repair pass on content before saving
        $content = Repair-PsbbnXml $content

        # CRITICAL: Write strictly in UTF-8 WITHOUT BOM (prevents PS2 C/C++ parser crash)
        [System.IO.File]::WriteAllText($xf.FullName, $content, $utf8NoBom)
    }

    # Step 2b: Automated XML Syntax Validation
    Write-Host ""
    Write-Host ("  " + $ui.ValidatingXml) -ForegroundColor Cyan
    $docTester = New-Object System.Xml.XmlDocument
    $xmlErrors = 0
    foreach ($xf in $xmlFiles) {
        $xc = [System.IO.File]::ReadAllText($xf.FullName, [System.Text.Encoding]::UTF8)
        try {
            $docTester.LoadXml(($xc -replace '<\?xml\s+version="1\.1"', '<?xml version="1.0"'))
        } catch {
            $repaired = Repair-PsbbnXml $xc
            [System.IO.File]::WriteAllText($xf.FullName, $repaired, $utf8NoBom)
            try {
                $docTester.LoadXml(($repaired -replace '<\?xml\s+version="1\.1"', '<?xml version="1.0"'))
            } catch {
                $xmlErrors++
                Write-Host ("  " + ($ui.XmlSyntaxError -f $xf.Name, $_.Exception.Message)) -ForegroundColor Yellow
            }
        }
    }
    if ($xmlErrors -eq 0) {
        Write-Host ("  " + $ui.XmlValidationSuccess) -ForegroundColor Green
    }

    # Step 3: Translate HTML Help Files (accent-free for NetFront)
    $htmlFiles = Get-ChildItem -Path $destFolder -Recurse -Filter "*.html"
    Write-Host ""
    Write-Host ("  " + ($ui.Step3TranslatingAtok -f $htmlFiles.Count)) -ForegroundColor Cyan

    $allHtmlTexts = New-Object System.Collections.Generic.HashSet[string]
    foreach ($hf in $htmlFiles) {
        $content = [System.IO.File]::ReadAllText($hf.FullName, [System.Text.Encoding]::GetEncoding("Shift_JIS"))
        $matches = [regex]::Matches($content, '>([^<]+)<')
        foreach ($m in $matches) {
            $trimmed = $m.Groups[1].Value.Trim()
            if ($trimmed.Length -gt 0 -and ($trimmed -match '[a-zA-Z]')) {
                if (-not ($trimmed -match '^(&nbsp;|\s)*$')) {
                    [void]$allHtmlTexts.Add($trimmed)
                }
            }
        }
    }

    $missingHtml = New-Object System.Collections.Generic.List[string]
    foreach ($ht in $allHtmlTexts) {
        if (-not $cache.ContainsKey($ht) -or ($cache[$ht].Length -eq 1 -and $ht.Length -gt 2)) {
            $missingHtml.Add($ht)
        }
    }

    if ($missingHtml.Count -gt 0) {
        for ($b = 0; $b -lt $missingHtml.Count; $b += 15) {
            $chunk = @()
            $end = [Math]::Min($missingHtml.Count, $b + 15)
            for ($k = $b; $k -lt $end; $k++) { $chunk += $missingHtml[$k] }
            Show-InlineProgress $ui.ProgressAtokHelp $end $missingHtml.Count
            [string[]]$bRes = @(Invoke-BatchTranslation $chunk $targetLangObj.google)
            for ($k = 0; $k -lt $chunk.Count; $k++) {
                $orig = $chunk[$k]
                $trans = if ($k -lt $bRes.Count -and $bRes[$k] -and (Is-ValidTranslation [string]$bRes[$k])) { [string]$bRes[$k] } else { $orig }
                if ($trans.Length -eq 1 -and $orig.Length -gt 2) {
                    $trans = $orig
                }
                $noAccents = Remove-Diacritics $trans
                if (Is-ValidTranslation $noAccents) {
                    $cache[$orig] = $noAccents
                }
            }
            if (($b % 75) -eq 0) {
                # Translation cache kept strictly in-memory during run; no stale disk persistence
    ...
    foreach ($hf in $htmlFiles) {
        $htmlCounter++
        Show-InlineProgress $ui.ProgressUpdatingHtml $htmlCounter $htmlFiles.Count
        ...
        $newContent = [regex]::Replace($content, '>([^<]+)<', {
            ...
            $trimmed = $inner.Trim()
            if ($trimmed.Length -gt 0 -and $cache.ContainsKey($trimmed) -and (Is-ValidTranslation $cache[$trimmed])) {
                $replacement = $cache[$trimmed]
                $leadIdx = $inner.IndexOf($trimmed)
                $lead = $inner.Substring(0, $leadIdx)
                $trail = $inner.Substring($leadIdx + $trimmed.Length)
                return ">$lead$replacement$trail<"
            }
            return $m.Value
        })
        [System.IO.File]::WriteAllText($hf.FullName, $newContent, [System.Text.Encoding]::GetEncoding("Shift_JIS"))
    }
    ...
    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Cyan
    Write-Host ("  " + $ui.PkgGenTitle) -ForegroundColor Cyan
    Write-Host "  ========================================================================================" -ForegroundColor Cyan
    Write-Host ("  " + $ui.PkgNoticeHeader) -ForegroundColor Yellow
    Write-Host ("  " + $ui.PkgNotice1) -ForegroundColor DarkGray
    Write-Host ("  " + $ui.PkgNotice2) -ForegroundColor DarkGray
    Write-Host ("  " + $ui.PkgNotice3) -ForegroundColor DarkGray
    Write-Host "  ----------------------------------------------------------------------------------------" -ForegroundColor DarkGray

    $createTar = $true
    if (-not $noWait) {
        $tarAnswer = Read-Host ("  " + $ui.PkgPromptTar + " ")
        if ($tarAnswer.Trim().ToLower() -eq "n") {
            $createTar = $false
        }
    }

    if ($createTar) {
        $tarRaw = Join-Path $destFolder "bnupdate.tar"
        $tarFile = Join-Path $destFolder "bnupdate.tar.gz"
        if (Test-Path $tarRaw) { Remove-Item $tarRaw -Force }
        if (Test-Path $tarFile) { Remove-Item $tarFile -Force }
        Write-Host ("  " + $ui.PkgCreatingTar) -ForegroundColor Cyan
        $tarCmd = "tar -cf `"$tarRaw`" -C `"$destFolder`" --format=v7 --exclude=`"bnupdate.tar*`" ."
        Invoke-Expression $tarCmd

        if (Test-Path $tarRaw) {
            Set-TarExecutablePermissions $tarRaw
            Write-Host ("  " + $ui.PkgCompressingGz) -ForegroundColor Cyan
            $inStream = [System.IO.File]::OpenRead($tarRaw)
            ...
            Remove-Item $tarRaw -Force -ErrorAction SilentlyContinue

            if (Test-Path $tarFile) {
                $tarSize = (Get-Item $tarFile).Length
                Write-Host ("  " + ($ui.PkgSuccess -f $tarFile, $tarSize)) -ForegroundColor Green
            }
        } else {
            Write-Host ("  " + $ui.PkgErrorTar) -ForegroundColor Yellow
        }
    }

    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.CompletionBanner -f "System PSBBN")) -ForegroundColor Green
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ""
    if (-not $noWait) { return (Wait-EndNavigation -inputDir $inputDir) }
    return "menu"
}
...
function Format-LauncherString([string]$val) {
...
function Build-LauncherLanguageBlock([string]$code3, $dict) {
    $sb = New-Object System.Text.StringBuilder
    [void]$sb.AppendLine("    $code3 = @{")

    for ($i = 1; $i -le 21; $i++) {
        $key = if ($i -lt 10) { "prompt_0$i" } else { "prompt_$i" }
        $v = if ($dict -and $dict.ContainsKey($key)) { [string]$dict[$key] } else { "" }
        [void]$sb.AppendLine("        $key = " + (Format-LauncherString $v))
    }
    [void]$sb.AppendLine("")
    ...
    for ($i = 1; $i -le 12; $i++) {
        $key = if ($i -lt 10) { "error_0$i" } else { "error_$i" }
        $v = if ($dict -and $dict.ContainsKey($key)) { [string]$dict[$key] } else { "" }
        [void]$sb.AppendLine("        $key = " + (Format-LauncherString $v))
    }
...
$Global:LauncherLocaleRules = @{
    "ara"     = @('    "ar*" { $winLang = "ara" }')
    "ben"     = @('    "bn*" { $winLang = "ben" }')
    "bul"     = @('    "bg*" { $winLang = "bul" }')
    "chi_sim" = @('    "zh-Hans*" { $winLang = "chi_sim" }', '    "zh-CN*" { $winLang = "chi_sim" }', '    "zh-SG*" { $winLang = "chi_sim" }')
    "chi_tra" = @('    "zh-Hant*" { $winLang = "chi_tra" }', '    "zh-TW*" { $winLang = "chi_tra" }', '    "zh-HK*" { $winLang = "chi_tra" }')
    "hrv"     = @('    "hr*" { $winLang = "hrv" }')
    "ces"     = @('    "cs*" { $winLang = "ces" }')
    "dan"     = @('    "da*" { $winLang = "dan" }')
    "nld"     = @('    "nl*" { $winLang = "nld" }')
    "fil"     = @('    "fil*" { $winLang = "fil" }', '    "tl*" { $winLang = "fil" }')
    "fin"     = @('    "fi*" { $winLang = "fin" }')
    "fre"     = @('    "fr*" { $winLang = "fre" }')
    "ger"     = @('    "de*" { $winLang = "ger" }')
    "ell"     = @('    "el*" { $winLang = "ell" }')
    "heb"     = @('    "he*" { $winLang = "heb" }', '    "iw*" { $winLang = "heb" }')
    "hin"     = @('    "hi*" { $winLang = "hin" }')
    "hun"     = @('    "hu*" { $winLang = "hun" }')
    "ind"     = @('    "id*" { $winLang = "ind" }', '    "in*" { $winLang = "ind" }')
    "ita"     = @('    "it*" { $winLang = "ita" }')
    "jpn"     = @('    "ja*" { $winLang = "jpn" }')
    "kor"     = @('    "ko*" { $winLang = "kor" }')
    "msa"     = @('    "ms*" { $winLang = "msa" }')
    "mar"     = @('    "mr*" { $winLang = "mar" }')
    "nor"     = @('    "nb*" { $winLang = "nor" }', '    "nn*" { $winLang = "nor" }', '    "no*" { $winLang = "nor" }')
    "fas"     = @('    "fa*" { $winLang = "fas" }')
    "pol"     = @('    "pl*" { $winLang = "pol" }')
    "por_pt"  = @('    "pt-PT*" { $winLang = "por_pt" }')
    "por"     = @('    "pt*" { $winLang = "por" }')
    "ron"     = @('    "ro*" { $winLang = "ron" }')
    "rus"     = @('    "ru*" { $winLang = "rus" }')
    "srp"     = @('    "sr*" { $winLang = "srp" }')
...
function Invoke-ModuleLauncherWindows($targetLangs = $null, [switch]$noWait = $false) {
    $ui = Get-ScriptUI
    try { $Host.UI.RawUI.WindowTitle = if ($ui.LauncherTitle) { $ui.LauncherTitle } else { "PSBBN Launcher for Windows Translator [By Emerson Teles]" } } catch { }

    $baseDir = Join-Path $ScriptDir "Launcher_Windows"
    ...
    foreach ($d in @($inputDir, $outputDir, $dataDir, $individualDir)) {
        if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d -Force | Out-Null }
    }

    $sourceFile = Join-Path $inputDir "PSBBN-Launcher-For-Windows.ps1"
    $destFile = Join-Path $outputDir "PSBBN-Launcher-For-Windows.ps1"
    $dataFile = Join-Path $dataDir "launcher_text_40langs.json"


    $launcherOnlineUrl = "https://github.com/CosmicScale/PSBBN-Definitive-English-Patch/releases/download/latest/PSBBN-Launcher-For-Windows.ps1"

    # Sempre baixa a versao mais recente oficial do GitHub release (substituindo o arquivo)
    Write-Host ""
    Write-Host ("  " + $ui.DownloadingLatestGithub) -ForegroundColor Cyan
    Write-Host "      Link: $launcherOnlineUrl" -ForegroundColor DarkGray
    try {
        [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12 -bor [System.Net.SecurityProtocolType]::Tls11 -bor [System.Net.SecurityProtocolType]::Tls
        $wc = New-Object System.Net.WebClient
        $wc.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64)")
        $downloaded = $wc.DownloadString($launcherOnlineUrl)
        if ($downloaded -and $downloaded.Contains('switch -wildcard ($winLang)')) {
            try {
                $utf8Enc = New-Object System.Text.UTF8Encoding($true)
                [System.IO.File]::WriteAllText($sourceFile, $downloaded, $utf8Enc)
            } catch { }
            Write-Host ("  " + ($ui.OfficialDownloadSuccess -f (Get-Item $sourceFile).Length)) -ForegroundColor Green
        }
    } catch {
    ...
    if (-not (Test-Path $sourceFile)) {
        $localCandidates = @(
            (Join-Path $ScriptDir "_Docs\_PSBBN Language update\PSBBN-Launcher-For-Windows.ps1"),
            (Join-Path $ScriptDir "_PSBBN Language update\PSBBN-Launcher-For-Windows.ps1"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_Docs\_PSBBN Language update\PSBBN-Launcher-For-Windows.ps1"),
            (Join-Path (Split-Path -Parent $ScriptDir) "_PSBBN Language update\PSBBN-Launcher-For-Windows.ps1")
        )
        foreach ($cand in $localCandidates) {
            if (Test-Path $cand) {
                try { Copy-Item -Path $cand -Destination $sourceFile -Force | Out-Null; break } catch { }
    ...
    if (Test-Path $sourceFile) {
        $scriptContent = [System.IO.File]::ReadAllText($sourceFile, [System.Text.Encoding]::UTF8)
    }

    if (-not $scriptContent) {
        Write-Host ""
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + $ui.LauncherBaseNotFoundTitle) -ForegroundColor Red
        Write-Host "  ========================================================================================" -ForegroundColor Red
        Write-Host ("  " + $ui.GenericEnsureInternetOrInput) -ForegroundColor Yellow
        Write-Host ""
        Write-Host ("  " + $ui.PressAnyKey) -ForegroundColor Cyan
        [void][Console]::ReadKey($true)
        return "back"
    ...
    if ($scriptContent.StartsWith([char]0xFEFF)) { $scriptContent = $scriptContent.Substring(1) }

    # Extract official keys and English defaults dynamically
    $engKeys = [ordered]@{}
    $engBlockMatch = [regex]::Match($scriptContent, '(?ms)\beng\s*=\s*@\{(.*?)\r?\n\s*\}')
    if ($engBlockMatch.Success) {
        $engBlockText = $engBlockMatch.Groups[1].Value
        foreach ($rawLine in ($engBlockText -split '\r?\n')) {
            if ($rawLine -match '^\s*([a-zA-Z0-9_]+)\s*=\s*["''](.*)["'']\s*$') {
                $k = $matches[1].Trim()
                $v = $matches[2]
                $engKeys[$k] = $v
            }
    ...
    if (Test-Path $dataFile) {
        try {
            ...
            $dictAll = $serializer.DeserializeObject($rawJson)
        } catch { }
    }

    # Normalize target languages selection
    $selectedLangs = New-Object System.Collections.Generic.List[object]
    if ($null -eq $targetLangs -or $targetLangs -eq "A" -or $targetLangs -eq "all" -or ($targetLangs -is [System.Collections.ICollection] -and $targetLangs.Count -eq 0)) {
        foreach ($l in $Languages40) { $selectedLangs.Add($l) }
    } elseif ($targetLangs -is [System.Collections.IEnumerable] -and -not ($targetLangs -is [string])) {
        foreach ($item in $targetLangs) {
            if ($item) { $selectedLangs.Add($item) }
        }
    } else {
        $selectedLangs.Add($targetLangs)
    }

    $isAll = ($selectedLangs.Count -eq $Languages40.Count)

    Write-Host ""
    Show-HeaderBanner "PSBBN LAUNCHER FOR WINDOWS TRANSLATOR"
    $langDesc = if ($isAll) {
        "Todos os 40 Idiomas (Multilíngue Completo)"
    } elseif ($selectedLangs.Count -eq 1) {
        $l0 = $selectedLangs[0]
        $l0Name = if ($l0.name) { $l0.name } else { $l0['name'] }
        $l0Nat = if ($l0.native) { $l0.native } else { $l0['native'] }
        "$l0Name ($l0Nat)"
    } else {
        ("$($selectedLangs.Count) idiomas selecionados (" + (($selectedLangs | ForEach-Object { if ($_.code3) { $_.code3 } else { $_['code3'] } }) -join ", ") + ")")
    }
    Write-Host ("  {0} {1}" -f $ui.Translating, $langDesc) -ForegroundColor White
    Write-Host ("  {0} {1}" -f $ui.SourceFile, $sourceFile) -ForegroundColor DarkGray
    Write-Host ("  {0} {1}" -f $ui.DestFile, $destFile) -ForegroundColor DarkGray
    Write-Host ("  " + ($ui.IndividualBlocks -f $individualDir)) -ForegroundColor DarkGray
    Write-Host ""

    # Always read from original clean input script so each run resets context to the base
    $scriptContent = [System.IO.File]::ReadAllText($sourceFile, [System.Text.Encoding]::UTF8)
    ...
    # Clean individual output folder so it only contains the text blocks of this specific run
    if (Test-Path $individualDir) {
        Get-ChildItem -Path $individualDir -Filter "*.txt" | Remove-Item -Force -ErrorAction SilentlyContinue
    }

    $totalLangs = $selectedLangs.Count

    for ($i = 0; $i -lt $totalLangs; $i++) {
        $l = $selectedLangs[$i]
        $code3 = if ($l.code3) { $l.code3 } else { $l['code3'] }
        $lName = if ($l.name) { $l.name } else { $l['name'] }
        $lNative = if ($l.native) { $l.native } else { $l['native'] }
        $lCode = if ($l.code) { $l.code } else { $l['code'] }
        Show-InlineProgress $ui.Translating ($i + 1) $totalLangs

        # Check if dictionary has this language
        $langData = if ($dictAll -and $dictAll.ContainsKey($code3)) { $dictAll[$code3] } else { $null }
        if (-not $langData) { continue }

        # Generate block for this language
        $blockStr = Build-LauncherLanguageBlock $code3 $langData

        # 1. Generate individual file for CosmicScale manual integration
        $indivFile = Join-Path $individualDir ("{0}.txt" -f $code3)
        $rulesForLang = if ($Global:LauncherLocaleRules.ContainsKey($code3)) { $Global:LauncherLocaleRules[$code3] -join "`r`n" } else { "    `"$($lCode.ToLower())*`" { `$winLang = `"$code3`" }" }
        $indivSb = New-Object System.Text.StringBuilder
        [void]$indivSb.AppendLine("<#")
        [void]$indivSb.AppendLine("================================================================================")
        [void]$indivSb.AppendLine(("PSBBN Launcher for Windows - Bloco de Tradução: {0} ({1})" -f $lName, $lNative))
        [void]$indivSb.AppendLine(("Código: {0} | Locale Windows: {1}" -f $code3, $lCode))
        [void]$indivSb.AppendLine("Autor: Emerson Teles (CosmicScale)")
        [void]$indivSb.AppendLine("================================================================================")
        [void]$indivSb.AppendLine("")
        [void]$indivSb.AppendLine("1) Copie a regra abaixo e cole dentro de 'switch -wildcard (`$winLang)'")
        [void]$indivSb.AppendLine("   (logo antes de 'default { `$winLang = `"eng`" }'):")
        [void]$indivSb.AppendLine("")
        [void]$indivSb.AppendLine($rulesForLang)
        [void]$indivSb.AppendLine("")
        [void]$indivSb.AppendLine("2) Copie o bloco abaixo e cole dentro de '`$TEXT = @{'")
        [void]$indivSb.AppendLine("   (logo abaixo do último idioma existente, antes de fechar com '}'):")
        [void]$indivSb.AppendLine("#>")
        [void]$indivSb.AppendLine("")
        [void]$indivSb.AppendLine($blockStr.TrimEnd())
        [void]$indivSb.AppendLine("")

        $indivUtf8 = New-Object System.Text.UTF8Encoding($true)
        [System.IO.File]::WriteAllText($indivFile, $indivSb.ToString(), $indivUtf8)

        # 2. Update switch -wildcard ($winLang) in main script (always place translated rules at the end)
        if ($code3 -eq "por" -or $code3 -eq "por_pt") {
            # Remove any existing pt-PT* or pt* rules from anywhere in switch
            $scriptContent = [regex]::Replace($scriptContent, '(?m)^\s*"pt-PT\*"\s*\{\s*\$winLang\s*=\s*"por_pt"\s*\}\r?\n', '')
            $scriptContent = [regex]::Replace($scriptContent, '(?m)^\s*"pt\*"\s*\{\s*\$winLang\s*=\s*"por"\s*\}\r?\n', '')

            # Place at the very bottom right before default { $winLang = "eng" }
            $ptRules = '    "pt-PT*" { $winLang = "por_pt" }' + "`r`n" + '    "pt*" { $winLang = "por" }'
            $scriptContent = [regex]::Replace($scriptContent, '(\r?\n\s*default\s*\{\s*\$winLang\s*=\s*"eng"\s*\})', "`r`n$ptRules`$1")
        } else {
            # Remove any existing rule for this language
            if ($Global:LauncherLocaleRules.ContainsKey($code3)) {
                foreach ($rule in $Global:LauncherLocaleRules[$code3]) {
                    $rulePattern = '(?m)^\s*' + [regex]::Escape($rule.Trim()) + '\r?\n'
                    $scriptContent = [regex]::Replace($scriptContent, $rulePattern, '')
                    $scriptContent = [regex]::Replace($scriptContent, '(\r?\n\s*default\s*\{\s*\$winLang\s*=\s*"eng"\s*\})', "`r`n$rule`$1")
                }
            } else {
                $genericRule = "    `"$($lCode.ToLower())*`" { `$winLang = `"$code3`" }"
                $rulePattern = '(?m)^\s*"' + [regex]::Escape($lCode.ToLower()) + '\*"\s*\{\s*\$winLang\s*=\s*"' + [regex]::Escape($code3) + '"\s*\}\r?\n'
                $scriptContent = [regex]::Replace($scriptContent, $rulePattern, '')
                $scriptContent = [regex]::Replace($scriptContent, '(\r?\n\s*default\s*\{\s*\$winLang\s*=\s*"eng"\s*\})', "`r`n$genericRule`$1")
            }
        }

        # 3. Always append at the very end of $TEXT = @{ ... } (remove any old block first)
        $existingRegex = "(?ms)^\s*" + [regex]::Escape($code3) + "\s*=\s*@\{.*?\r?\n\s*\}\r?\n?"
        if ($scriptContent -match $existingRegex) {
            $scriptContent = [regex]::Replace($scriptContent, $existingRegex, '')
        }

        # Insert right before the last closing brace of $TEXT before "# --- Simple text function ---"
        $marker = "# --- Simple text function ---"
        $idx = $scriptContent.IndexOf($marker)
        if ($idx -gt 0) {
            $beforeMarker = $scriptContent.Substring(0, $idx)
            $afterMarker = $scriptContent.Substring($idx)
            $lastBraceIdx = $beforeMarker.LastIndexOf("}")
            if ($lastBraceIdx -gt 0) {
                $insertBlock = "`r`n`r`n" + $blockStr.TrimEnd() + "`r`n"
                $scriptContent = $beforeMarker.Substring(0, $lastBraceIdx) + $insertBlock + $beforeMarker.Substring($lastBraceIdx) + $afterMarker
            }
        }
        ...
        # If any new keys were dynamically translated, persist updated dictionary to cache
    if ($dictModified) {
        try {
            Add-Type -AssemblyName System.Web.Extensions
            $serializer = New-Object System.Web.Script.Serialization.JavaScriptSerializer
            $serializer.MaxJsonLength = [int]::MaxValue
            $newJson = $serializer.Serialize($dictAll)
            $utf8Bom = New-Object System.Text.UTF8Encoding($true)
            [System.IO.File]::WriteAllText($dataFile, $newJson, $utf8Bom)
        } catch { }
    }

    # Write out the updated script with UTF-8 BOM

    # Write out the updated script with UTF-8 BOM
    $utf8Bom = New-Object System.Text.UTF8Encoding($true)
    [System.IO.File]::WriteAllText($destFile, $scriptContent, $utf8Bom)

    Write-Host ""
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.CompletionBanner -f "Launcher Windows")) -ForegroundColor Green
    Write-Host "  ========================================================================================" -ForegroundColor Green
    Write-Host ("  " + ($ui.LauncherIntegrated -f $selectedLangs.Count)) -ForegroundColor Green
    Write-Host ("  " + $ui.LauncherAutoDetect) -ForegroundColor Green
    Write-Host ("  " + ($ui.GeneratedFile -f $destFile)) -ForegroundColor White
    Write-Host ("  " + ($ui.IndividualBlocksNotice -f $individualDir)) -ForegroundColor Yellow
    Write-Host ""
    if (-not $noWait) { return (Wait-EndNavigation -inputFiles @($sourceFile)) }
    return "menu"
}
...
# ------------------------------------------------------------------------------
# Master Interactive Hub Loop
# ------------------------------------------------------------------------------

function Show-LanguageGrid($moduleName, $moduleDesc, $sourcePath, $moduleKey = $null) {
    $ui = Get-ScriptUI
    Clear-Host
    Show-HeaderBanner $moduleName
    $sessStr = ("{0} {1}" -f $ui.SessionStarted, (Get-SessionStartTime)).Trim()
    $padSess = [Math]::Max(0, [int][Math]::Floor((88 - $sessStr.Length) / 2))
    Write-Host ((" " * $padSess) + $sessStr) -ForegroundColor DarkGray
    Write-Host ("  {0}" -f $moduleDesc) -ForegroundColor White
    Write-Host ("  {0} {1}" -f $ui.SourceFile, $sourcePath) -ForegroundColor DarkGray
    Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
    ...
    Write-Host ("  [B] {0}" -f $ui.BackMenu) -ForegroundColor Yellow
    if ($moduleKey -eq "launcher") {
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        Write-Host ("  {0}" -f $ui.LauncherTipHeader) -ForegroundColor Yellow
        Write-Host ("    {0}" -f $ui.LauncherTip1) -ForegroundColor Yellow
        Write-Host ("    {0}" -f $ui.LauncherTipMulti) -ForegroundColor Yellow
        Write-Host ("    {0}" -f $ui.LauncherTipRange) -ForegroundColor Yellow
        Write-Host ("    {0}" -f $ui.LauncherTipAll) -ForegroundColor Yellow
    }
    Write-Host "========================================================================================" -ForegroundColor Cyan
}

function Invoke-LanguageSelection($moduleKey, $moduleName, $moduleDesc, $sourcePath) {
    while ($true) {
        Show-LanguageGrid $moduleName $moduleDesc $sourcePath $moduleKey
        $ui = Get-ScriptUI
        Write-Host ""
        $promptLabel = if ($moduleKey -eq "launcher") {
            "  $($ui.LauncherPrompt)"
        } else {
            "  $($ui.ChooseOption)"
        }
        $choice = Read-Host $promptLabel
        $choice = $choice.Trim()
        ...
        if ($choice -eq "L" -or $choice -eq "l") {
            $nav = Select-ScriptUILanguage
            if ($nav -eq "menu") { return }
            continue
        }

        if ($choice -eq "A" -or $choice -eq "a") {
            if ($moduleKey -eq "launcher") {
                $nav = Invoke-ModuleLauncherWindows $Languages40
            } else {
                foreach ($langObj in $Languages40) {
                    switch ($moduleKey) {
                        "readme"          { Invoke-ModuleReadme $langObj -noWait $true }
                        "changelog_main"  { Invoke-ModuleChangelogMain $langObj -noWait $true }
                        "changelog_patch" { Invoke-ModuleChangelogPatch $langObj -noWait $true }
                        "script"          { Invoke-ModuleScriptPsbbn $langObj -noWait $true }
                        "system"          { Invoke-ModuleSystemPsbbn $langObj -noWait $true }
                    }
                }
                Write-Host ""
                Write-Host "  ========================================================================================" -ForegroundColor Green
                Write-Host ("  " + ($ui.CompletionBanner -f $moduleName)) -ForegroundColor Green
                Write-Host "  ========================================================================================" -ForegroundColor Green
                Write-Host ""
                Write-Host ("  " + $ui.NavInstructions) -ForegroundColor Cyan
                $nav = Wait-EndNavigation
            }
            if ($nav -eq "back") { continue }
            return
        }

        # Parse comma-separated, range, or single input (e.g. "1,7,10", "1-5", "por", "fas")
        ...
        if ($selectedList.Count -gt 0) {
            $nav = $null
            if ($moduleKey -eq "launcher") {
                $nav = Invoke-ModuleLauncherWindows $selectedList
            } else {
                ...
                for ($idx = 0; $idx -lt $selectedList.Count; $idx++) {
                    ...
                    switch ($moduleKey) {
                        "readme"          { $nav = Invoke-ModuleReadme $item -noWait $noWaitFlag }
                        "changelog_main"  { $nav = Invoke-ModuleChangelogMain $item -noWait $noWaitFlag }
                        "changelog_patch" { $nav = Invoke-ModuleChangelogPatch $item -noWait $noWaitFlag }
                        "script"          { $nav = Invoke-ModuleScriptPsbbn $item -noWait $noWaitFlag }
                        "system"          { $nav = Invoke-ModuleSystemPsbbn $item -noWait $noWaitFlag }
                    }
                }
            }
            if ($nav -eq "back") {
...
function Show-MasterMenu {
    while ($true) {
        $ui = Get-ScriptUI
        try { $Host.UI.RawUI.WindowTitle = $ui.SuiteTitle } catch { }

        Clear-Host
        Show-HeaderBanner $ui.SuiteTitle

        # Sessao Iniciada centralizada embaixo do titulo
        $sessStr = ("{0} {1}" -f $ui.SessionStarted, (Get-SessionStartTime)).Trim()
        $padSess = [Math]::Max(0, [int][Math]::Floor((88 - $sessStr.Length) / 2))
        Write-Host ((" " * $padSess) + $sessStr) -ForegroundColor DarkGray

        # Aviso de Texturas com quebra de linha limpa e centralizado
        $cleanDisc = ($ui.TextureDisclaimer -replace '\s+', ' ').Trim()
        $discLines = New-Object System.Collections.Generic.List[string]
        if ($cleanDisc -match '^(.{25,}?[.;!\u3002])\s+(.+)$') {
            $discLines.Add($matches[1].Trim())
            $discLines.Add($matches[2].Trim())
        } else {
            $discLines.Add($cleanDisc)
        }
        foreach ($dl in $discLines) {
            $padDisc = [Math]::Max(0, [int][Math]::Floor((88 - $dl.Length) / 2))
            Write-Host ((" " * $padDisc) + $dl) -ForegroundColor DarkYellow
        }
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        Write-Host "  [1] " -NoNewline -ForegroundColor White; Write-Host "Changelog Main" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescMain) -ForegroundColor DarkGray
        Write-Host "  [2] " -NoNewline -ForegroundColor White; Write-Host "Changelog Patch" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescPatch) -ForegroundColor DarkGray
        Write-Host "  [3] " -NoNewline -ForegroundColor White; Write-Host "Readme" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescReadme) -ForegroundColor DarkGray
        Write-Host "  [4] " -NoNewline -ForegroundColor White; Write-Host "Script PSBBN" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescScript) -ForegroundColor DarkGray
        Write-Host "  [5] " -NoNewline -ForegroundColor White; Write-Host "System PSBBN" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescSystem) -ForegroundColor DarkGray
        Write-Host "  [6] " -NoNewline -ForegroundColor White; Write-Host "Launcher for Windows" -ForegroundColor Yellow
        Write-Host ("      {0}" -f $ui.DescLauncher) -ForegroundColor DarkGray
        Write-Host "----------------------------------------------------------------------------------------" -ForegroundColor DarkGray
        Write-Host ("  [L] {0}" -f $ui.ChangeLanguage) -ForegroundColor Magenta
        ...
        switch ($choice) {
            "1" { Invoke-LanguageSelection "changelog_main" $ui.ChangelogMainTitle $ui.DescMain "Changelog_Main\input\changelog_main_eng.txt" }
            "2" { Invoke-LanguageSelection "changelog_patch" $ui.ChangelogPatchTitle $ui.DescPatch "Changelog_Patch\input\changelog_patch_eng.txt" }
            "3" { Invoke-LanguageSelection "readme" $ui.ReadmeTitle $ui.DescReadme "Readme\input\README.md" }
            "4" { Invoke-LanguageSelection "script" $ui.ScriptTitle $ui.DescScript "Script PSBBN\input\eng.txt" }
            "5" { Invoke-LanguageSelection "system" $ui.SystemTitle $ui.DescSystem "System PSBBN\input\PSBBN-translation-pack-v3.5\PSBBN_English" }
            "6" { Invoke-LanguageSelection "launcher" $ui.LauncherTitle $ui.DescLauncher "Launcher_Windows\input\PSBBN-Launcher-For-Windows.ps1" }
            "l" { $nav = Select-ScriptUILanguage; continue }
            "L" { $nav = Select-ScriptUILanguage; continue }
            "0" { exit 0 }
            default {
                Write-Host ""
...
# ------------------------------------------------------------------------------
# Main Entry Point & CLI Parameter Handler
# ------------------------------------------------------------------------------
Enable-ConsoleQuickEdit

if ($Module -and $Lang) {
    $sel = $null
    foreach ($l in $Languages40) {
        if ($Lang.ToLower() -eq $l.code.ToLower() -or $Lang.ToLower() -eq $l.code3.ToLower() -or $Lang -eq $l.id) {
            $sel = $l
            break
        }
    }
    if ($sel) {
        switch ($Module.ToLower()) {
            "readme"          { Invoke-ModuleReadme $sel -noWait $NoWait; exit 0 }
            "changelog_main"  { Invoke-ModuleChangelogMain $sel -noWait $NoWait; exit 0 }
            "changelog_patch" { Invoke-ModuleChangelogPatch $sel -noWait $NoWait; exit 0 }
            "script"          { Invoke-ModuleScriptPsbbn $sel -noWait $NoWait; exit 0 }
            "system"          { Invoke-ModuleSystemPsbbn $sel -noWait $NoWait; exit 0 }
            "launcher"        { Invoke-ModuleLauncherWindows $sel -noWait $NoWait; exit 0 }
        }
    }
}
